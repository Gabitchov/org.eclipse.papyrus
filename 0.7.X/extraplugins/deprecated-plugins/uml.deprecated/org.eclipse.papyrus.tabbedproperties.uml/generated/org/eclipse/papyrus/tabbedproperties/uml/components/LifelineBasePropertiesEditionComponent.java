...
public class LifelineBasePropertiesEditionComponent extends StandardPropertiesEditionComponent {
	 ...
	 * Default constructor
	 */
	public LifelineBasePropertiesEditionComponent(EObject lifeline, String editing_mode) {
	...
	private AdapterImpl initializeSemanticAdapter() {
		return new EContentAdapter() {
			...
			public void notifyChanged(Notification msg) {
				if(basePart == null)
					LifelineBasePropertiesEditionComponent.this.dispose();
				else {
	...
	public void setPropertiesEditionPart(java.lang.Class key, int kind, IPropertiesEditionPart propertiesEditionPart) {
		if(key == UMLViewsRepository.Lifeline.class)
			this.basePart = (LifelinePropertiesEditionPart)propertiesEditionPart;
	}
	...
	public CompoundCommand getPropertiesEditionCommand(EditingDomain editingDomain) {
		...
		if(lifeline != null) {
			...
			//}
			List coveredByToAddFromCoveredBy = basePart.getCoveredByToAdd();
			for(Iterator iter = coveredByToAddFromCoveredBy.iterator(); iter.hasNext();)
				cc.append(AddCommand.create(editingDomain, lifeline, UMLPackage.eINSTANCE.getLifeline_CoveredBy(), iter.next()));
			List coveredByToRemoveFromCoveredBy = basePart.getCoveredByToRemove();
			for(Iterator iter = coveredByToRemoveFromCoveredBy.iterator(); iter.hasNext();)
				cc.append(RemoveCommand.create(editingDomain, lifeline, UMLPackage.eINSTANCE.getLifeline_CoveredBy(), iter.next()));
			//List coveredByToMoveFromCoveredBy = basePart.getCoveredByToMove();
			//for (Iterator iter = coveredByToMoveFromCoveredBy.iterator(); iter.hasNext();){
			//	org.eclipse.emf.eef.runtime.impl.utils.EMFListEditUtil.MoveElement moveElement = (org.eclipse.emf.eef.runtime.impl.utils.EMFListEditUtil.MoveElement)iter.next();
	...
	public Diagnostic validate() {
		...
		if(IPropertiesEditionComponent.BATCH_MODE.equals(editing_mode)) {
			...
			validate = Diagnostician.INSTANCE.validate(lifeline);
