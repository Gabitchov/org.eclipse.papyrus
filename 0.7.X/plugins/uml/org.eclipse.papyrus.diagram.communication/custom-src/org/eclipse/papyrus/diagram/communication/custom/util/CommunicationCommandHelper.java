...
public class CommunicationCommandHelper {
	...
	public static Message doCreateMessage(Interaction container, MessageSort messageSort, Element source, Element target, MessageEnd sendMessageEnd, MessageEnd receiveMessageEnd) {
		...
		if(sendMessageEnd != null) {
			...
			org.eclipse.papyrus.diagram.communication.custom.providers.ElementInitializers.init_NamedElement(sendMessageEnd, "", message.getName(), "Send"); //$NON-NLS-1$ //$NON-NLS-2$
		...
		if(receiveMessageEnd != null) {
			...
			org.eclipse.papyrus.diagram.communication.custom.providers.ElementInitializers.init_NamedElement(receiveMessageEnd, "", message.getName(), "Recv"); //$NON-NLS-1$ //$NON-NLS-2$
			// Update the message with the messages end
	...
	public static Message doCreateMessage(Interaction interaction, MessageSort messageSort, NamedElement signature) {
		...
		// Init Name
		if(signature == null) {
			org.eclipse.papyrus.diagram.communication.custom.providers.ElementInitializers.init_NamedElement(message);
		} else {
