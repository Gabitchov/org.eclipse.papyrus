...
import org.eclipse.papyrus.diagram.clazz.custom.policies.itemsemantic.CustomSignalItemSemanticEditPolicy;
...

{
	...
	protected void createDefaultEditPolicies() {
		...
		installEditPolicy(EditPolicyRoles.SEMANTIC_ROLE,
				new CustomSignalItemSemanticEditPolicy());
		installEditPolicy(
	...
	protected boolean addFixedChild(EditPart childEditPart) {
		...
		if (childEditPart instanceof SignalAttributeCompartmentEditPart) {
			...
			pane.add(((SignalAttributeCompartmentEditPart) childEditPart)
					.getFigure());
			return true;
		}
	...
	protected boolean removeFixedChild(EditPart childEditPart) {
		...
		if (childEditPart instanceof SignalAttributeCompartmentEditPart) {
			IFigure pane = getPrimaryShape().getAttributeCompartmentFigure();
			setupContentPane(pane); // FIXME each comparment should handle his content pane in his own way 
			pane.remove(((SignalAttributeCompartmentEditPart) childEditPart)
					.getFigure());
			return true;
		}
		if (childEditPart instanceof RedefinableTemplateSignatureEditPart) {
	...
	public List<IElementType> getMARelTypesOnSourceAndTarget(
		...
		if (targetEditPart instanceof org.eclipse.papyrus.diagram.clazz.edit.parts.SignalEditPart) {
			types.add(UMLElementTypes.AssociationClass_4017);
		}
		...
		if (feature == NotationPackage.eINSTANCE.getLineStyle_LineColor()
			...
			if (feature == NotationPackage.eINSTANCE.getLineStyle_LineColor()) {
				prefColor = PreferenceConstantHelper.getElementConstant(
						"Signal", PreferenceConstantHelper.COLOR_LINE);
			} else if (feature == NotationPackage.eINSTANCE
					.getFontStyle_FontColor()) {
				prefColor = PreferenceConstantHelper.getElementConstant(
						"Signal", PreferenceConstantHelper.COLOR_FONT);
			} else if (feature == NotationPackage.eINSTANCE
					.getFillStyle_FillColor()) {
				prefColor = PreferenceConstantHelper.getElementConstant(
						"Signal", PreferenceConstantHelper.COLOR_FILL);
			}
			...
			String prefGradient = PreferenceConstantHelper.getElementConstant(
					"Signal", PreferenceConstantHelper.COLOR_GRADIENT);
