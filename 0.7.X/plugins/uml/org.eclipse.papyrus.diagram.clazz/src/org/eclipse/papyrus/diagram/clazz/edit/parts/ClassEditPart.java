...
import org.eclipse.papyrus.diagram.clazz.custom.locator.ContainmentPositionLocator;
...

{
	...
	protected boolean addFixedChild(EditPart childEditPart) {
		...
		if (childEditPart instanceof ClassNestedClassifierCompartment2EditPart) {
			...
			setupContentPane(pane); // FIXME each comparment should handle his content pane in his own way 
			pane.add(((ClassNestedClassifierCompartment2EditPart) childEditPart)
					.getFigure());
			return true;
		}
		...
		if (childEditPart instanceof ContainmentCircleEditPart) {
			IBorderItemLocator locator = new ContainmentPositionLocator(
					getMainFigure(), PositionConstants.NORTH);
			getBorderedFigure().getBorderItemContainer().add(
					((ContainmentCircleEditPart) childEditPart).getFigure(),
					locator);
			return true;
		}
	...
	protected void createDefaultEditPolicies() {
		...
		installEditPolicy(EditPolicyRoles.SEMANTIC_ROLE,
				new CustomClassItemSemanticEditPolicy());
		installEditPolicy(
	...
	public List<IElementType> getMARelTypesOnSourceAndTarget(
		...
		if (targetEditPart instanceof org.eclipse.papyrus.diagram.clazz.edit.parts.ClassEditPart) {
			types.add(UMLElementTypes.AssociationClass_4017);
		}
		...
		if (feature == NotationPackage.eINSTANCE.getLineStyle_LineColor()
			...
			if (feature == NotationPackage.eINSTANCE.getLineStyle_LineColor()) {
				prefColor = PreferenceConstantHelper.getElementConstant(
						"Class", PreferenceConstantHelper.COLOR_LINE);
			} else if (feature == NotationPackage.eINSTANCE
					.getFontStyle_FontColor()) {
				prefColor = PreferenceConstantHelper.getElementConstant(
						"Class", PreferenceConstantHelper.COLOR_FONT);
			} else if (feature == NotationPackage.eINSTANCE
					.getFillStyle_FillColor()) {
				prefColor = PreferenceConstantHelper.getElementConstant(
						"Class", PreferenceConstantHelper.COLOR_FILL);
			}
			...
			String prefGradient = PreferenceConstantHelper.getElementConstant(
					"Class", PreferenceConstantHelper.COLOR_GRADIENT);
	...
	protected boolean removeFixedChild(EditPart childEditPart) {
		...
		if (childEditPart instanceof ClassNestedClassifierCompartment2EditPart) {
			IFigure pane = getPrimaryShape().getNestedClassifierFigure();
			setupContentPane(pane); // FIXME each comparment should handle his content pane in his own way 
			pane.remove(((ClassNestedClassifierCompartment2EditPart) childEditPart)
					.getFigure());
			return true;
		}
		if (childEditPart instanceof RedefinableTemplateSignatureEditPart) {
