...
ARCHIVE_INDEX=/home/data/httpd/download.eclipse.org/modeling/mdt/papyrus/downloads/index.html
...
if [ $signalDateTrunkNightly -gt $lastPromoteDateTrunkNightly ]; then
	...
	promote "$zipName" "$version" "$nfsURL" "$hudsonURL" "$DROPS_DIR" "$ARCHIVE_DIR" "$ARCHIVE_INDEX" "$UPDATES_NIGHTLY" "Papyrus-Update-" "NA"
