...
public class DurationConstraintBasePropertiesEditionComponent extends StandardPropertiesEditionComponent {
		...
		return new EContentAdapter() {
			...
			public void notifyChanged(Notification msg) {
				if (basePart == null)
					...
					// FIXME invalid case in liveUpdater(), Case : model = Attribute(*) : firstEvent
					// - view = MultiValuedEditor
	...
	public IPropertiesEditionPart getPropertiesEditionPart(int kind, String key) {
		if (durationConstraint != null && BASE_PART.equals(key)) {
			if (basePart == null) {
				...
				if (provider != null) {
					basePart = (DurationConstraintPropertiesEditionPart) provider.getPropertiesEditionPart(
							UMLViewsRepository.DurationConstraint.class, kind, this);
	...
	public void initPart(java.lang.Class key, int kind, EObject elt, ResourceSet allResource) {
		if (basePart != null && key == UMLViewsRepository.DurationConstraint.class) {
			...
			basePart.initVisibility((EEnum) UMLPackage.eINSTANCE.getNamedElement_Visibility().getEType(),
					durationConstraint.getVisibility());
			basePart.initClientDependency(durationConstraint, null, UMLPackage.eINSTANCE
					.getNamedElement_ClientDependency());
			basePart.initConstrainedElement(durationConstraint, null, UMLPackage.eINSTANCE
			...
			// viewCommon.mtl module, with the values : firstEvent, DurationConstraint,
			// DurationConstraint.
	...
	public CompoundCommand getPropertiesEditionCommand(EditingDomain editingDomain) {
		...
		if (durationConstraint != null) {
			...
			for (Iterator iter = ownedCommentToAddFromOwnedComment.iterator(); iter.hasNext();)
				cc.append(AddCommand.create(editingDomain, durationConstraint, UMLPackage.eINSTANCE
			...
			for (Iterator iter = ownedCommentToMoveFromOwnedComment.iterator(); iter.hasNext();) {
				org.eclipse.emf.eef.runtime.impl.utils.EMFListEditUtil.MoveElement moveElement = (org.eclipse.emf.eef.runtime.impl.utils.EMFListEditUtil.MoveElement) iter
						.next();
				cc.append(MoveCommand.create(editingDomain, durationConstraint, UMLPackage.eINSTANCE.getComment(),
						moveElement.getElement(), moveElement.getIndex()));
			}
			cc.append(SetCommand.create(editingDomain, durationConstraint, UMLPackage.eINSTANCE.getNamedElement_Name(),
					basePart.getName()));

			cc.append(SetCommand.create(editingDomain, durationConstraint, UMLPackage.eINSTANCE
			...
			for (Iterator iter = clientDependencyToRemoveFromClientDependency.iterator(); iter.hasNext();)
				cc.append(RemoveCommand.create(editingDomain, durationConstraint, UMLPackage.eINSTANCE
						.getNamedElement_ClientDependency(), iter.next()));
			...
			// cc.append(MoveCommand.create(editingDomain, durationConstraint,
			...
			// moveElement.getIndex()));
			// }
			// FIXME invalid case in commandUpdater(), Case : model = Attribute(*) : firstEvent -
			// view = MultiValuedEditor
	...
	public EObject getPropertiesEditionObject(EObject source) {
		if (source instanceof DurationConstraint) {
			...
			// FIXME invalid case in partUpdater(), Case : model = Attribute(*) : firstEvent - view
			// = MultiValuedEditor
		...
		if (PropertiesEditionEvent.COMMIT == event.getState()
			...
			if (UMLViewsRepository.DurationConstraint.ownedComment == event.getAffectedEditor()) {
				if (PropertiesEditionEvent.SET == event.getKind()) {
					...
					command.append(AddCommand.create(liveEditingDomain, durationConstraint, UMLPackage.eINSTANCE
							.getElement_OwnedComment(), event.getNewValue()));
				else if (PropertiesEditionEvent.REMOVE == event.getKind())
					command.append(DeleteCommand.create(liveEditingDomain, event.getNewValue()));
				else if (PropertiesEditionEvent.MOVE == event.getKind())
					command.append(MoveCommand.create(liveEditingDomain, durationConstraint, UMLPackage.eINSTANCE
							.getComment(), event.getNewValue(), event.getNewIndex()));
			}
			if (UMLViewsRepository.DurationConstraint.name == event.getAffectedEditor())
				command.append(SetCommand.create(liveEditingDomain, durationConstraint, UMLPackage.eINSTANCE
			...
			if (UMLViewsRepository.DurationConstraint.clientDependency == event.getAffectedEditor()) {
				...
				if (PropertiesEditionEvent.REMOVE == event.getKind())
					command.append(RemoveCommand.create(liveEditingDomain, durationConstraint, UMLPackage.eINSTANCE
							.getNamedElement_ClientDependency(), event.getNewValue()));
			...
			// FIXME invalid case in liveCommandUpdater(), Case : model = Attribute(*) : firstEvent
			// - view = MultiValuedEditor
	...
	public Diagnostic validateValue(PropertiesEditionEvent event) {
		...
		try {
			...
			if (UMLViewsRepository.DurationConstraint.firstEvent == event.getAffectedEditor()) {
				Object newValue = EcoreUtil.createFromString(UMLPackage.eINSTANCE.getDurationConstraint_FirstEvent()
						.getEAttributeType(), newStringValue);
				ret = Diagnostician.INSTANCE.validate(UMLPackage.eINSTANCE.getDurationConstraint_FirstEvent()
						.getEAttributeType(), newValue);
			}
