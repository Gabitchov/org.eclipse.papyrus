...
public class AbstractionBasePropertiesEditionComponent extends StandardPropertiesEditionComponent {
	...
	public IPropertiesEditionPart getPropertiesEditionPart(int kind, String key) {
		if (abstraction != null && BASE_PART.equals(key)) {
			if (basePart == null) {
				...
				if (provider != null) {
					basePart = (AbstractionPropertiesEditionPart) provider.getPropertiesEditionPart(
							UMLViewsRepository.Abstraction.class, kind, this);
	...
	public void initPart(java.lang.Class key, int kind, EObject elt, ResourceSet allResource) {
		if (basePart != null && key == UMLViewsRepository.Abstraction.class) {
			...
			basePart.initVisibility((EEnum) UMLPackage.eINSTANCE.getNamedElement_Visibility().getEType(), abstraction
					.getVisibility());
	...
	public CompoundCommand getPropertiesEditionCommand(EditingDomain editingDomain) {
		...
		if (abstraction != null) {
			cc.append(SetCommand.create(editingDomain, abstraction, UMLPackage.eINSTANCE.getNamedElement_Name(),
					basePart.getName()));

			cc.append(SetCommand.create(editingDomain, abstraction, UMLPackage.eINSTANCE.getNamedElement_Visibility(),
			...
			for (Iterator iter = supplierToAddFromSupplier.iterator(); iter.hasNext();)
				cc.append(AddCommand.create(editingDomain, abstraction, UMLPackage.eINSTANCE.getDependency_Supplier(),
						iter.next()));
			...
			for (Iterator iter = supplierToRemoveFromSupplier.iterator(); iter.hasNext();)
				cc.append(RemoveCommand.create(editingDomain, abstraction, UMLPackage.eINSTANCE
			...
			// cc.append(MoveCommand.create(editingDomain, abstraction,
			...
			for (Iterator iter = clientToAddFromClient.iterator(); iter.hasNext();)
				cc.append(AddCommand.create(editingDomain, abstraction, UMLPackage.eINSTANCE.getDependency_Client(),
						iter.next()));
			List clientToRemoveFromClient = basePart.getClientToRemove();
			for (Iterator iter = clientToRemoveFromClient.iterator(); iter.hasNext();)
				cc.append(RemoveCommand.create(editingDomain, abstraction, UMLPackage.eINSTANCE.getDependency_Client(),
						iter.next()));
	...
	public void firePropertiesChanged(PropertiesEditionEvent event) {
		...
		if (PropertiesEditionEvent.COMMIT == event.getState()
			...
			if (UMLViewsRepository.Abstraction.name == event.getAffectedEditor())
				command.append(SetCommand.create(liveEditingDomain, abstraction, UMLPackage.eINSTANCE
			...
			if (UMLViewsRepository.Abstraction.supplier == event.getAffectedEditor()) {
				if (PropertiesEditionEvent.ADD == event.getKind())
					command.append(AddCommand.create(liveEditingDomain, abstraction, UMLPackage.eINSTANCE
							.getDependency_Supplier(), event.getNewValue()));
				if (PropertiesEditionEvent.REMOVE == event.getKind())
					command.append(RemoveCommand.create(liveEditingDomain, abstraction, UMLPackage.eINSTANCE
							.getDependency_Supplier(), event.getNewValue()));
				if (PropertiesEditionEvent.MOVE == event.getKind())
					command.append(MoveCommand.create(liveEditingDomain, abstraction, UMLPackage.eINSTANCE
