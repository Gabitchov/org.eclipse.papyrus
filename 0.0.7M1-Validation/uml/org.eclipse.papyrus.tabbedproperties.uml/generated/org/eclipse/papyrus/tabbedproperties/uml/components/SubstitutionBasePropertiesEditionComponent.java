...
public class SubstitutionBasePropertiesEditionComponent extends StandardPropertiesEditionComponent {
	...
	public IPropertiesEditionPart getPropertiesEditionPart(int kind, String key) {
		if (substitution != null && BASE_PART.equals(key)) {
			if (basePart == null) {
				...
				if (provider != null) {
					basePart = (SubstitutionPropertiesEditionPart) provider.getPropertiesEditionPart(
							UMLViewsRepository.Substitution.class, kind, this);
	...
	public void initPart(java.lang.Class key, int kind, EObject elt, ResourceSet allResource) {
		if (basePart != null && key == UMLViewsRepository.Substitution.class) {
			...
			basePart.initVisibility((EEnum) UMLPackage.eINSTANCE.getNamedElement_Visibility().getEType(), substitution
					.getVisibility());
	...
	public CompoundCommand getPropertiesEditionCommand(EditingDomain editingDomain) {
		...
		if (substitution != null) {
			cc.append(SetCommand.create(editingDomain, substitution, UMLPackage.eINSTANCE.getNamedElement_Name(),
					basePart.getName()));

			cc.append(SetCommand.create(editingDomain, substitution, UMLPackage.eINSTANCE.getNamedElement_Visibility(),
			...
			for (Iterator iter = supplierToAddFromSupplier.iterator(); iter.hasNext();)
				cc.append(AddCommand.create(editingDomain, substitution, UMLPackage.eINSTANCE.getDependency_Supplier(),
						iter.next()));
			...
			for (Iterator iter = supplierToRemoveFromSupplier.iterator(); iter.hasNext();)
				cc.append(RemoveCommand.create(editingDomain, substitution, UMLPackage.eINSTANCE
			...
			// cc.append(MoveCommand.create(editingDomain, substitution,
			...
			for (Iterator iter = clientToAddFromClient.iterator(); iter.hasNext();)
				cc.append(AddCommand.create(editingDomain, substitution, UMLPackage.eINSTANCE.getDependency_Client(),
						iter.next()));
			List clientToRemoveFromClient = basePart.getClientToRemove();
			for (Iterator iter = clientToRemoveFromClient.iterator(); iter.hasNext();)
				cc.append(RemoveCommand.create(editingDomain, substitution,
						UMLPackage.eINSTANCE.getDependency_Client(), iter.next()));
	...
	public void firePropertiesChanged(PropertiesEditionEvent event) {
		...
		if (PropertiesEditionEvent.COMMIT == event.getState()
			...
			if (UMLViewsRepository.Substitution.name == event.getAffectedEditor())
				command.append(SetCommand.create(liveEditingDomain, substitution, UMLPackage.eINSTANCE
			...
			if (UMLViewsRepository.Substitution.supplier == event.getAffectedEditor()) {
				if (PropertiesEditionEvent.ADD == event.getKind())
					command.append(AddCommand.create(liveEditingDomain, substitution, UMLPackage.eINSTANCE
							.getDependency_Supplier(), event.getNewValue()));
				if (PropertiesEditionEvent.REMOVE == event.getKind())
					command.append(RemoveCommand.create(liveEditingDomain, substitution, UMLPackage.eINSTANCE
							.getDependency_Supplier(), event.getNewValue()));
				if (PropertiesEditionEvent.MOVE == event.getKind())
					command.append(MoveCommand.create(liveEditingDomain, substitution, UMLPackage.eINSTANCE
