 ...
 * 
 */
public class TilePartContainer {
	 ...
	 * Constructor.
	 */
	public TilePartContainer(IMultiEditorNestedPartManager nestedPartManager, ISashWindowsModelManager model) {
		this.nestedPartManager = nestedPartManager;
		this.model = model;
		this.tilePartFactory = new TilePartFactory(this);
	}
	 ...
	 */
	protected ITilePartFactory getTilePartFactory() {
	 ...
	 */
	protected ISashWindowsModelManager getContainerModel() {
		return model;
	}
	 ...
	 */
	private DropTarget createDropTarget(final TabFolderPart sourcePart, int srcTabIndex, int side, int cursor, ITilePart targetPart) {
		if (dropTarget == null) {
	...
	public void synchronize2() {
		System.out.println("synchronize2()");

		container.setRedraw(false);
		// Create map of parts
		PartMap partMap = new PartMap();
		rootPart.fillPartMap(partMap);

		// Synchronize parts
		rootPart.synchronize2(partMap);

		// Remove orphaned part (no more used)
		for (ITilePart part : partMap.getTileParts()) {
			if (part.isOrphaned())
				part.dispose(false);
		}
	...
	public void synchronize() {
		...
		// Create the tree of tile part.
		rootPart.createPartControl(container);
		// rootPart.getControl().setRedraw(true);
		container.setRedraw(true);
		container.layout(true, true);
	}; // end class
	...
	protected void createRootPart() {
		rootPart = new RootPart(getTilePartFactory(), model);
	}
	...
	protected class DropTarget implements IDropTarget {
		 ...
		 * Constructor.
		 */
		public DropTarget(TabFolderPart sourcePart, int srcTabIndex, int cursor, int side, ITilePart targetPart) {
			this.cursor = cursor;
			this.side = side;
		...
		public void setTarget(TabFolderPart sourcePart, int srcTabIndex, int cursor, int side, ITilePart targetPart) {
			this.cursor = cursor;
			this.side = side;
			this.sourcePart = sourcePart;
			this.srcTabIndex = srcTabIndex;
		...
		public void drop() {
			...
			if (side == SWT.CENTER) { // Add to target folder
				model.moveTab(sourcePart.getModel(), srcTabIndex, ((TabFolderPart) targetPart).getModel(), -1);
			} else { // Create a new folder
				model.createFolder(sourcePart.getModel(), srcTabIndex, ((TabFolderPart) targetPart).getModel(), side);
			}
		}
		...
		protected float getDockingRatio(ITilePart dragged, ITilePart target) {
			return 0.5f;
		}
