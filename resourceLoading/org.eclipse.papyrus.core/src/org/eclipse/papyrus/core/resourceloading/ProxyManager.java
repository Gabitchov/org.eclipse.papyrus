...
public class ProxyManager {
	...
	private boolean loadResource(URI uri) {
		boolean result = availableStrategies.get(getCurrentStrategy()).loadResource(diResourceSet, uri);
		...
		while(!result && iterator.hasNext()) {
			...
			result = extension.loadResource(diResourceSet, uri.trimFragment());
		}
		return result;
	}
	...
	public EObject getEObjectFromStrategy(URI uri) throws MissingResourceException {
		...
		if(loadOnDemand) {
			...
			Resource resource = diResourceSet.getResource(trimFragment, loadOnDemand);
			if(resource != null) {
				...
				// RouteManager should be used for that
					...
					if(DiResourceSet.DI_FILE_EXTENSION.equals(fileExtension)) {
						...
						diResource = diResourceSet.getResource(trimFragment, loadOnDemand);
						...
						URI newURI = trimFragment.trimFileExtension().appendFileExtension(DiResourceSet.DI_FILE_EXTENSION);
						diResource = diResourceSet.getResource(newURI.trimFragment(), loadOnDemand);
