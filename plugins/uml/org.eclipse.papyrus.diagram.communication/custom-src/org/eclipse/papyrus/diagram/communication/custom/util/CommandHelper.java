...
public class CommandHelper {
	...
	/**
	 * Create a message on the given interaction. It only creates the message
	 * and not its messages end.
	 * 
	 * @param interaction
	 ...
	 * 
	 * @param es
	 *        the ExecutionSpecification associated with this
	 *        ExecutionOccurrenceSpecification.
	 * @param event
	 *        the event associated with this OccurrenceSpecification. It
	 *        must be of type ExecutionEvent.
	 * @param fragment
	 *        the fragment enclosing this OccurenceSpecifcation. It must be
	 *        an Interaction or an Operand.
	 * @param lifeline
	 ...
	 * @param fragment
	 *        the fragment containing the {@link OccurrenceSpecification}.
	 *        It can be an {@link Interaction} or an {@link InteractionOperand}
	 * @param lifeline
	...
	/**
	 * Get the signature of the message. Opens a dialog box to select a
	 * signature.
	 * 
	 * @param message
	 *        The message
	 * @return null, if cancel has been pressed. An empty list if the null
	 *         Element has been selected, or a list with the selected element.
	 */
	...
	/**
	 * Get the signature of the message. Opens a dialog box to select a
	 * signature. Inputs depends on the messageSort, if any.
	 * 
	 * @param message
	 *        The message
	 * @param messageSort
	...
	public static List<NamedElement> getSignature(Element message, MessageSort messageSort) {
		...
		dialog.setTitle(Messages.CommandHelper_signatureslection);
		dialog.setMessage(Messages.CommandHelper_SelectSignature);
	...
	public static Property getProperties(List<Property> availableProperties) {
		...
		ILabelProvider labelProvider = new AdapterFactoryLabelProvider(UMLDiagramEditorPlugin.getInstance().getItemProvidersAdapterFactory());
		ElementListSelectionDialog dialog = new ElementListSelectionDialog(Display.getCurrent().getActiveShell(), labelProvider);
		dialog.setTitle(Messages.CommandHelper_PropertySelection);
		dialog.setMessage(Messages.CommandHelper_SelectProperty);
	 ...
	 * According to the UML Specification, an ExecutionSpecification must have a
	 * start and a finish.
	 * 
	 * In Papyrus, an ExecutionSpecification may not have a start and finish. To
	 * handle this specific case, we return the first lifeline that the ES
	 * covered.
	 * 
	 * @param es
	 *        the targeted execution specification
	 * @return the associated lifeline, or null if the ES has no start, no
	 *         finish, no covered lifelines.
	 */
	...
	/**
	 * Create an ExecutionSpecification. It also creates the start and finish
	 * ExecutionOccurenceSpecification of the ExecutionSpecification, and their
	 ...
	 * @return A MessageOccurrenceSpecification if element is
	 *         ExecutionSpecification or Lifeline. A Gate if element is
	 *         Interaction or CombinedFragment or InteractionUse
	 */
	...
	/**
	 * Create gate if element is a Interaction, a Combined Fragment or a
	 * Interaction Use
	 * 
	 * @param element
	 *        The element
	...
	/**
	 * Create a message. It also creates its message end, their corresponding
	 * events and updates the signature of the message.
	 * 
	 * @param container
	...
	public static Message doCreateMessage(Interaction container, MessageSort messageSort, Element source, Element target, InteractionFragment sourceContainer, InteractionFragment targetContainer) {
		...
		// If signatures == null, means the user click on cancel button during
		// selection --> Cancel the whole process of creation
		if(signatures == null) {
			return null;
		}
	...
	/**
	 * Get the messageSort of a message if it doesn't exist yet depending of the
	 * messageSignature. If no messageSort exists, and if the signature is null,
	 * then return a MessageSort.ASYNCH_CALL_LITERAL
	 * 
	 * @param signature
