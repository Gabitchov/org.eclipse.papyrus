 ...
 * long as the proper Classes are provided.
 * <br>
 * It needs the IExtensionPoint string ID. It needs an Array of Classes to
 * create instances of that kind. If the Classes are internal to another
 * Class, an instance of the enclosing Class is required.
 * <br>
 * The Classes provided must follow these rules:
 * <ul>
 * <li>Each Class simple name must be equal to one of the elements defined in
 * the IExtensionPoint to be parsed.</li>
 * <li>There must be one Class for each one of the elements defined in the
 * IExtensionPoint.</li>
 * <li>Each Class must have one attribute of type String, Boolean or Object for
 * each attribute defined for its matching element in the IExtensionPoint.</li>
 * <li>Each Class must have one attribute of type List for each type of
 * child element it can have as defined in the IExtensionPoint. The name of
 * this List attribute will be equal to the type of elements to be stored in
 * it.</li>
 * <li>Use Boolean instead of boolean, this allows that field to be null,
 * indicating it has not been initialized.</li>
 * </ul>
 * <br>
 * The method parseExtensionPoint will return a List of Objects with a data
 * structure similar to the one defined in the IExtensionPoint.
 * 
...
public class ExtensionPointParser {
	 ...
	 */
	public ExtensionPointParser(String extensionPointID,
			Class<Object>[] classes, Object enclosingInstance) {
		this.classes = classes;
		this.extensionPointID = extensionPointID;
	...
	public List<Object> parseExtensionPoint() {
		if (getExtensionPointID() == null || getClasses() == null
				|| getClasses().length <= 0) {
		...
		if (extensionPoint == null) {
			Activator.getDefault().logError(
					"The Extension Point ID provided does not exist", null);
			return Collections.EMPTY_LIST;
		}
	 ...
	 * @return
	 */
	protected Object parseAttribute(IConfigurationElement element,
			String attribute) {
		...
		for (Class<Object> clazz : getClasses()) {
			if (clazz.getSimpleName().equals(name)) {
				try {
					...
					Constructor<Object> constructor = clazz
