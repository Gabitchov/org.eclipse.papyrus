...
/**
 * The defaul palette provider. It reads XML palette contributions from the provider's extension
 * point and contributes them to an editor's palette based on different contribution criteria
 * 
 ...
 */
public class PapyrusPaletteProvider extends AbstractProvider implements IPaletteProvider {
	...
	private static class ContributionDescriptor {
		 ...
		 *            map of predefined palette entries where the key is the palette entry id and
		 *            the value is the palette entry
		 */
		public void contribute(Object content, PaletteRoot root, Map predefinedEntries, String pluginID) {

			IPreferenceStore store = Activator.getDefault().getPreferenceStore();
			String sValue = store.getString("palette.config." + pluginID);
			XMLMemento rootMemento = null;
			if (sValue != null && !sValue.equals("")) { //$NON-NLS-1$
				try {
					rootMemento = XMLMemento.createReadRoot(new StringReader(sValue));
			...
			contribute(content, root, predefinedEntries, pluginID, rootMemento);
		...
		/**
		 * Contribute to the palette, using the configuration file
		 * 
		 * @param content
		 * @param root
		 * @param predefinedEntries
		 ...
		 * @param rootMemento
		 */
		protected void contribute(Object content, PaletteRoot root, Map predefinedEntries, String pluginID,
	...
	private static interface IEntryDescriptor {
		...
		/**
		 * Contributes the palette entry based on the given content, starting from the given root
		 * and into the given path
		 * 
		 * @param content
		 * @param root
	...
	private static class EntryDescriptor implements IEntryDescriptor {
		...
		public EntryDescriptor(IConfigurationElement configElement) {
			...
			if (DRAWER.equals(kindStr))
				...
				Log.info(DiagramProvidersPlugin.getInstance(), DiagramProvidersStatusCodes.SERVICE_FAILURE,
						"No factory class name is provided"); //$NON-NLS-1$
			...
			if (path == null && !defineOnly)
				Log.info(DiagramProvidersPlugin.getInstance(), DiagramProvidersStatusCodes.SERVICE_FAILURE,
						"Path must be provided when contributing a palette entry"); //$NON-NLS-1$
			...
			if (NONE.equals(kindStr))
				permission = new Integer(PaletteEntry.PERMISSION_NO_MODIFICATION);
			if (HIDEONLY.equals(kindStr))
				permission = new Integer(PaletteEntry.PERMISSION_HIDE_ONLY);
			if (LIMITED.equals(kindStr))
				permission = new Integer(PaletteEntry.PERMISSION_LIMITED_MODIFICATION);
			if (FULL.equals(kindStr))
				permission = new Integer(PaletteEntry.PERMISSION_FULL_MODIFICATION);
		...
		private ImageDescriptor findIconImageDescriptor(IConfigurationElement configElement, String iconPath) {
			...
			try {
				...
				Trace.catching(DiagramUIPlugin.getInstance(), DiagramUIDebugOptions.EXCEPTIONS_CATCHING,
						PapyrusPaletteProvider.class, e.getLocalizedMessage(), e);
		...
		public void contribute(Object content, PaletteRoot root, PaletteFactoryProxy paletteFactory,
			...
			if (paletteEntry != null) {
				...
				if (permission != null)
					paletteEntry.setUserModificationPermission(permission.intValue());
	...
	private static void appendPaletteEntry(PaletteRoot root, Map predefinedEntries, String path,
		...
		if (fEntry == null)
			Log.info(DiagramProvidersPlugin.getInstance(), DiagramProvidersStatusCodes.SERVICE_FAILURE,
					"Invalid palette entry path"); //$NON-NLS-1$                
	...
	/**
	 * A proxy for a palette factory that instantiates the real factory on demand (when a palette
	 * entry is selcted)
	 */
