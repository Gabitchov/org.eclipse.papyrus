...
public abstract class AbstractParser implements IParser {
	...
	protected IParserEditStatus validateNewValues(Object[] values) {
		...
		for (int i = 0; i < values.length; i++) {
			...
			if (value instanceof InvalidValue) {
				return new ParserEditStatus(org.eclipse.papyrus.diagram.clazzBis.part.UMLClazzDiagramEditorPlugin.ID, IParserEditStatus.UNEDITABLE, value.toString());
			}
		}
		...
		if (type instanceof EDataType) {
			...
			if (Boolean.TYPE.equals(iClass)) {
				if (value instanceof Boolean) {
					...
					value = new InvalidValue(NLS.bind(org.eclipse.papyrus.diagram.clazzBis.part.Messages.AbstractParser_UnexpectedValueType, iClass.getName()));
				}
				...
				if (value instanceof Byte) {
					...
					if (s.length() == 0) {
						...
						try {
							...
							value = new InvalidValue(NLS.bind(org.eclipse.papyrus.diagram.clazzBis.part.Messages.AbstractParser_WrongStringConversion, iClass.getName()));
						}
					}
				} else {
					value = new InvalidValue(NLS.bind(org.eclipse.papyrus.diagram.clazzBis.part.Messages.AbstractParser_UnexpectedValueType, iClass.getName()));
				...
				if (value instanceof String) {
					...
					if (literal == null) {
						value = new InvalidValue(NLS.bind(org.eclipse.papyrus.diagram.clazzBis.part.Messages.AbstractParser_UnknownLiteral, value));
					} else {
						value = literal.getInstance();
					}
				} else {
					value = new InvalidValue(NLS.bind(org.eclipse.papyrus.diagram.clazzBis.part.Messages.AbstractParser_UnexpectedValueType, String.class.getName()));
				}
			}
		}
		return value;
