...
public class MultiAssociationHelper {
	 ...
	 * @param association
	 * @return the property concerned by the branch or null if the property is
	 *         not found
	 */
	public static Property getPropertyToListen(Edge associationBranchView,
			Association association) {
		Classifier target = (Classifier) (associationBranchView.getTarget()
				.getElement());
		// look for the property that is typed by the classifier
		Property propertyToDisplay = null;
		Iterator<Property> propertiesIterator = association.getMemberEnds()
				.iterator();
	 ...
	 * 
	 * @param editDomain
	 *            the editDomain , important to create commands
	 */
	...
	private Command getAssociationToMultiAssociationCommand(
		...
		if (targetEditPart != null) {
			...
			if (associationView == null
					|| (targetEditPart.getModel() instanceof Edge)) {
				return null;
			}
	...
	private Command getBranchAssociationCommand(
		...
		if (targetEditPart != null) {
			...
			CustomDeferredCreateConnectionViewCommand aBranchCommand = new CustomDeferredCreateConnectionViewCommand(
					...
					((IHintedType) UMLElementTypes.Association_4019)
							.getSemanticHint(), new SemanticAdapter(null,
							sourceEditPart.getModel()), new SemanticAdapter(
							null, targetEditPart.getModel()), sourceEditPart
							.getViewer(), ((IGraphicalEditPart) sourceEditPart)
							.getDiagramPreferencesHint(), viewDescriptor, null);
	...
	/**
	 * return commands to transform a binary association to n-ary association or
	...
	public Command getCommand(
		...
		if ((((View) sourceEditPart.getModel()).getType() == ""
			...
			return getBranchAssociationCommand(
