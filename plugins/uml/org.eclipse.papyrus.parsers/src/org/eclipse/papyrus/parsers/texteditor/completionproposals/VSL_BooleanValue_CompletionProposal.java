...
public class VSL_BooleanValue_CompletionProposal implements ICompletionProposalComputer {
	...
	final static private String[] TVL_BooleanValue_Strings = {
			"true",
			"false",
	};

	/** Text displayed in the information window */
	final static private String[] TVL_BooleanValue_StringsInfo = {
			NFPMessages.BooleanValue_True_StringsInfo,
			NFPMessages.BooleanValue_False_StringsInfo,
	}; 

	/** Text displayed in the completion area window */
	final static private String[] TVL_BooleanValue_StringName = {
		NFPMessages.BooleanValue_True_StringName,
		...
		for(int i=0; i < TVL_BooleanValue_Strings.length; i++) {
			if(TVL_BooleanValue_Strings[i].startsWith(prefix)) {
				proposal = new CompletionProposal(
						TVL_BooleanValue_Strings[i],
						documentOffset-prefix.length(),
						prefix.length()+selectionRange,
						TVL_BooleanValue_Strings[i].length(),
						null,
						TVL_BooleanValue_StringName[i],
						null,
						TVL_BooleanValue_StringsInfo[i]);
				v.add(proposal);
			}
		}
