...
public class VSL_NumberLiteral_CompletionProposal implements ICompletionProposalComputer {
	...
	final static private String[] TVL_NumberLiteral_StringsInfo = {
			NFPMessages.NumberLiteral_Integer_StringsInfo,
			NFPMessages.NumberLiteral_Real_StringsInfo,
	}; 

	/** Text displayed in the completion area window */
	final static private String[] TVL_NumberLiteral_StringName = {
		NFPMessages.NumberLiteral_Integer_StringName,
		...
		for(int i=0; i < TVL_NumberLiteral_Strings.length; i++) {
			if(TVL_NumberLiteral_Strings[i].startsWith(prefix)) {
				proposal = new CompletionProposal(
						TVL_NumberLiteral_Strings[i],
						documentOffset-prefix.length(),
						prefix.length()+selectionRange,
						TVL_NumberLiteral_Strings[i].length(),
						null,
						TVL_NumberLiteral_StringName[i],
						null,
						TVL_NumberLiteral_StringsInfo[i]);
				v.add(proposal);
			}
		}
