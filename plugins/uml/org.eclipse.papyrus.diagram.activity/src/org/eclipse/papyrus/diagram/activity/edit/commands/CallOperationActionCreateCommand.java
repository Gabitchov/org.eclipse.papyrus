...
public class CallOperationActionCreateCommand extends EditElementCommand {
	 ...
	 * @generated
	 */
	public static CallOperationActionCreateCommand create(
	...
	public boolean canExecute() {
		...
		return CreateCommandUtil
				.canCreateNode(getRequest(), getElementToEdit());
	}
	...
	protected CommandResult doExecuteWithResult(IProgressMonitor monitor,
		...
		CallOperationAction newElement = UMLFactory.eINSTANCE
				.createCallOperationAction();
		if (isOperationAlreadyManaged()) {
			CreateCallOperationActionDialog dialog = new CreateCallOperationActionDialog(
					Display.getDefault().getActiveShell(), parentActivity);
			if (IDialogConstants.OK_ID == dialog.open()) {
				// initialize the invoked element (no need to use a command,
				// since action is being created)
		...
		if (!CreateCommandUtil.setNodeParents(newElement, getRequest(),
				getElementToEdit())) {
			return CommandResult.newCancelledCommandResult();
		}
		...
		ElementInitializers.getInstance().init_CallOperationAction_3010(
				newElement);
	...
	private boolean isOperationAlreadyManaged() {
		...
		return !IFeatureSetterAspectActionUtil.areFeaturesManaged(getRequest(),
	 ...
	 * @generated
	 */
	protected void doConfigure(CallOperationAction newElement,
