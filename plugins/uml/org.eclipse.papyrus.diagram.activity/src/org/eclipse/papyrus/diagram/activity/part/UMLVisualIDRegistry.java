...
public class UMLVisualIDRegistry {
	...
	public static int getNodeVisualID(View containerView, EObject domainElement) {
		...
		case SendObjectActionEditPart.VISUAL_ID:
			...
			if (UMLPackage.eINSTANCE.getInputPin().isSuperTypeOf(domainElement.eClass())) {
				if (domainElement instanceof InputPin && domainElement.eContainer() instanceof SendObjectAction && ((SendObjectAction) domainElement.eContainer()).getTarget() == domainElement)
					return InputPinEditPart.VISUAL_ID;
			}
			if (UMLPackage.eINSTANCE.getInputPin().isSuperTypeOf(domainElement.eClass())) {
				if (domainElement instanceof InputPin && domainElement.eContainer() instanceof SendObjectAction && ((SendObjectAction) domainElement.eContainer()).getRequest() == domainElement)
		...
		case CallOperationActionEditPart.VISUAL_ID:
			...
			// request
			if (UMLPackage.eINSTANCE.getInputPin().isSuperTypeOf(domainElement.eClass())) {
				if (domainElement instanceof InputPin && domainElement.eContainer() instanceof CallOperationAction
						&& ((CallOperationAction) domainElement.eContainer()).getArguments().contains(domainElement))
					return InputPin5EditPart.VISUAL_ID;
			}
			if (UMLPackage.eINSTANCE.getInputPin().isSuperTypeOf(domainElement.eClass())) {
				if (domainElement instanceof InputPin && domainElement.eContainer() instanceof CallOperationAction && ((CallOperationAction) domainElement.eContainer()).getTarget() == domainElement)
	...
	public static ViewInfo getDiagramViewInfo() {
		if (diagramViewInfo == null) {
			diagramViewInfo = ViewInfoRegistry.getInstance().getHeadViewInfoForEditor(UMLDiagramEditor.ID);
		}
