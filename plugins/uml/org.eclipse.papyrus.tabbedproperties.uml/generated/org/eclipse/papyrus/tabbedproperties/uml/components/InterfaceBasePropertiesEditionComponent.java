...
public class InterfaceBasePropertiesEditionComponent extends StandardPropertiesEditionComponent {
	 ...
	 * The Base part
	 */
	private InterfacePropertiesEditionPart basePart;
	
	/**
	 * Default constructor
	 */
	public InterfaceBasePropertiesEditionComponent(EObject interface_, String mode) {
	...
	public java.lang.Class translatePart(String key) {
		if (BASE_PART.equals(key))
			return UMLViewsRepository.Interface.class;
	...
	public IPropertiesEditionPart getPropertiesEditionPart(int kind, String key) {
		if (interface_ != null && BASE_PART.equals(key)) {
			if (basePart == null) {
				...
				if (provider != null) {
					basePart = (InterfacePropertiesEditionPart)provider.getPropertiesEditionPart(UMLViewsRepository.Interface.class, kind, this);
	...
	public void initPart(java.lang.Class key, int kind, EObject elt, ResourceSet allResource) {
		if (basePart != null && key == UMLViewsRepository.Interface.class) {
	...
	public void firePropertiesChanged(PathedPropertiesEditionEvent event) {
		...
		if (PathedPropertiesEditionEvent.COMMIT == event.getState() && IPropertiesEditionComponent.LIVE_MODE.equals(mode)) {
			...
			if (UMLViewsRepository.Interface.name == event.getAffectedEditor())
			...
			if (UMLViewsRepository.Interface.visibility == event.getAffectedEditor())
			...
			if (UMLViewsRepository.Interface.elementImport == event.getAffectedEditor()) {
			...
			if (UMLViewsRepository.Interface.packageImport == event.getAffectedEditor()) {
				if (PathedPropertiesEditionEvent.SET == event.getKind()) {
			...
			if (UMLViewsRepository.Interface.ownedRule == event.getAffectedEditor()) {
				if (PathedPropertiesEditionEvent.SET == event.getKind()) {
			...
			if (UMLViewsRepository.Interface.isLeaf == event.getAffectedEditor())
			...
			if (UMLViewsRepository.Interface.templateBinding == event.getAffectedEditor()) {
				if (PathedPropertiesEditionEvent.SET == event.getKind()) {
			...
			if (UMLViewsRepository.Interface.isAbstract == event.getAffectedEditor())
			...
			if (UMLViewsRepository.Interface.generalization == event.getAffectedEditor()) {
				if (PathedPropertiesEditionEvent.SET == event.getKind()) {
			...
			if (UMLViewsRepository.Interface.powertypeExtent == event.getAffectedEditor()) {
			...
			if (UMLViewsRepository.Interface.redefinedClassifier == event.getAffectedEditor()) {
				if (PathedPropertiesEditionEvent.ADD == event.getKind())
			...
			if (UMLViewsRepository.Interface.substitution == event.getAffectedEditor()) {
				if (PathedPropertiesEditionEvent.SET == event.getKind()) {
			...
			if (UMLViewsRepository.Interface.collaborationUse == event.getAffectedEditor()) {
				if (PathedPropertiesEditionEvent.SET == event.getKind()) {
			...
			if (UMLViewsRepository.Interface.ownedUseCase == event.getAffectedEditor()) {
				if (PathedPropertiesEditionEvent.SET == event.getKind()) {
			...
			if (UMLViewsRepository.Interface.useCase == event.getAffectedEditor()) {
				if (PathedPropertiesEditionEvent.ADD == event.getKind())
			...
			if (UMLViewsRepository.Interface.ownedAttribute == event.getAffectedEditor()) {
				if (PathedPropertiesEditionEvent.SET == event.getKind()) {
			...
			if (UMLViewsRepository.Interface.ownedOperation == event.getAffectedEditor()) {
				if (PathedPropertiesEditionEvent.SET == event.getKind()) {
			...
			if (UMLViewsRepository.Interface.nestedClassifier == event.getAffectedEditor()) {
				if (PathedPropertiesEditionEvent.SET == event.getKind()) {
			...
			if (UMLViewsRepository.Interface.redefinedInterface == event.getAffectedEditor()) {
				if (PathedPropertiesEditionEvent.ADD == event.getKind())
			...
			if (UMLViewsRepository.Interface.ownedReception == event.getAffectedEditor()) {
				if (PathedPropertiesEditionEvent.SET == event.getKind()) {
	...
	public boolean isRequired(String key, int kind) {
		return key == UMLViewsRepository.Interface.isLeaf || key == UMLViewsRepository.Interface.isAbstract;
	}
	...
	public String getHelpContent(String key, int kind) {
			if (key == UMLViewsRepository.Interface.name)
				return "The name of the NamedElement."; //$NON-NLS-1$
			if (key == UMLViewsRepository.Interface.visibility)
				return "Determines where the NamedElement appears within different Namespaces within the overall model, and its accessibility."; //$NON-NLS-1$
			if (key == UMLViewsRepository.Interface.elementImport)
				return "References the ElementImports owned by the Namespace."; //$NON-NLS-1$
			if (key == UMLViewsRepository.Interface.packageImport)
				return "References the PackageImports owned by the Namespace."; //$NON-NLS-1$
			if (key == UMLViewsRepository.Interface.ownedRule)
				return "Specifies a set of Constraints owned by this Namespace."; //$NON-NLS-1$
			if (key == UMLViewsRepository.Interface.isLeaf)
				return "Indicates whether it is possible to further specialize a RedefinableElement. If the value is true, then it is not possible to further specialize the RedefinableElement."; //$NON-NLS-1$
			if (key == UMLViewsRepository.Interface.templateBinding)
				return "The optional bindings from this element to templates."; //$NON-NLS-1$
			if (key == UMLViewsRepository.Interface.isAbstract)
			...
			if (key == UMLViewsRepository.Interface.generalization)
