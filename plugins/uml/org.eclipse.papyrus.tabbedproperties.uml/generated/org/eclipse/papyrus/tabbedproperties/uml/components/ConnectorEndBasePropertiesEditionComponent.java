...
public class ConnectorEndBasePropertiesEditionComponent extends StandardPropertiesEditionComponent {
	...
	public IPropertiesEditionPart getPropertiesEditionPart(int kind, String key) {
		if (connectorEnd != null && BASE_PART.equals(key)) {
			if (basePart == null) {
				...
				if (provider != null) {
					basePart = (ConnectorEndPropertiesEditionPart) provider.getPropertiesEditionPart(
							UMLViewsRepository.ConnectorEnd.class, kind, this);
	...
	public void initPart(java.lang.Class key, int kind, EObject elt, ResourceSet allResource) {
		if (basePart != null && key == UMLViewsRepository.ConnectorEnd.class) {
			...
			ConnectorEnd connectorEnd = (ConnectorEnd) elt;
			// init values
	...
	public CompoundCommand getPropertiesEditionCommand(EditingDomain editingDomain) {
		...
		if (connectorEnd != null) {
			...
			for (Iterator iter = ownedCommentToAddFromOwnedComment.iterator(); iter.hasNext();)
				cc.append(AddCommand.create(editingDomain, connectorEnd,
			...
			for (Iterator iter = ownedCommentToMoveFromOwnedComment.iterator(); iter.hasNext();) {
				org.eclipse.emf.eef.runtime.impl.utils.EMFListEditUtil.MoveElement moveElement = (org.eclipse.emf.eef.runtime.impl.utils.EMFListEditUtil.MoveElement) iter
						.next();
				cc.append(MoveCommand.create(editingDomain, connectorEnd, UMLPackage.eINSTANCE.getComment(),
						moveElement.getElement(), moveElement.getIndex()));
			}
			cc.append(SetCommand.create(editingDomain, connectorEnd, UMLPackage.eINSTANCE
		...
		if (PropertiesEditionEvent.COMMIT == event.getState()
			...
			if (UMLViewsRepository.ConnectorEnd.ownedComment == event.getAffectedEditor()) {
				if (PropertiesEditionEvent.SET == event.getKind()) {
					...
					command.append(AddCommand.create(liveEditingDomain, connectorEnd, UMLPackage.eINSTANCE
							.getElement_OwnedComment(), event.getNewValue()));
				else if (PropertiesEditionEvent.REMOVE == event.getKind())
					command.append(DeleteCommand.create(liveEditingDomain, event.getNewValue()));
				else if (PropertiesEditionEvent.MOVE == event.getKind())
					command.append(MoveCommand.create(liveEditingDomain, connectorEnd, UMLPackage.eINSTANCE
							.getComment(), event.getNewValue(), event.getNewIndex()));
			}
			if (UMLViewsRepository.ConnectorEnd.isOrdered == event.getAffectedEditor())
				command.append(SetCommand.create(liveEditingDomain, connectorEnd, UMLPackage.eINSTANCE
	...
	public String getHelpContent(String key, int kind) {
		if (key == UMLViewsRepository.ConnectorEnd.ownedComment)
			return null; //$NON-NLS-1$
		if (key == UMLViewsRepository.ConnectorEnd.isOrdered)
			return null; //$NON-NLS-1$
		if (key == UMLViewsRepository.ConnectorEnd.isUnique)
			return null; //$NON-NLS-1$
	...
	public Diagnostic validate() {
		if (IPropertiesEditionComponent.BATCH_MODE.equals(editing_mode)) {
			...
			return Diagnostician.INSTANCE.validate(connectorEnd);
		else
			return null;
	}
