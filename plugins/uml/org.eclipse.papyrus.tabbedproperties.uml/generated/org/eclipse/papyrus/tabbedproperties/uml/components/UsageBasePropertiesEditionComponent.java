...
public class UsageBasePropertiesEditionComponent extends StandardPropertiesEditionComponent {
	...
	public IPropertiesEditionPart getPropertiesEditionPart(int kind, String key) {
		if (usage != null && BASE_PART.equals(key)) {
			if (basePart == null) {
				...
				if (provider != null) {
					basePart = (UsagePropertiesEditionPart) provider.getPropertiesEditionPart(
							UMLViewsRepository.Usage.class, kind, this);
	...
	public void initPart(java.lang.Class key, int kind, EObject elt, ResourceSet allResource) {
		if (basePart != null && key == UMLViewsRepository.Usage.class) {
			...
			basePart.initVisibility((EEnum) UMLPackage.eINSTANCE.getNamedElement_Visibility().getEType(), usage
					.getVisibility());
	...
	public CompoundCommand getPropertiesEditionCommand(EditingDomain editingDomain) {
		...
		if (usage != null) {
			cc.append(SetCommand.create(editingDomain, usage, UMLPackage.eINSTANCE.getNamedElement_Name(), basePart
					.getName()));

			cc.append(SetCommand.create(editingDomain, usage, UMLPackage.eINSTANCE.getNamedElement_Visibility(),
			...
			for (Iterator iter = supplierToAddFromSupplier.iterator(); iter.hasNext();)
				cc.append(AddCommand.create(editingDomain, usage, UMLPackage.eINSTANCE.getDependency_Supplier(), iter
						.next()));
			...
			for (Iterator iter = supplierToRemoveFromSupplier.iterator(); iter.hasNext();)
				cc.append(RemoveCommand.create(editingDomain, usage, UMLPackage.eINSTANCE.getDependency_Supplier(),
						iter.next()));
			...
			// cc.append(MoveCommand.create(editingDomain, usage,
			...
			for (Iterator iter = clientToAddFromClient.iterator(); iter.hasNext();)
				cc.append(AddCommand.create(editingDomain, usage, UMLPackage.eINSTANCE.getDependency_Client(), iter
						.next()));
			List clientToRemoveFromClient = basePart.getClientToRemove();
			for (Iterator iter = clientToRemoveFromClient.iterator(); iter.hasNext();)
				cc.append(RemoveCommand.create(editingDomain, usage, UMLPackage.eINSTANCE.getDependency_Client(), iter
						.next()));
	...
	public void firePropertiesChanged(PropertiesEditionEvent event) {
		...
		if (PropertiesEditionEvent.COMMIT == event.getState()
			...
			if (UMLViewsRepository.Usage.name == event.getAffectedEditor())
				command.append(SetCommand.create(liveEditingDomain, usage, UMLPackage.eINSTANCE.getNamedElement_Name(),
						event.getNewValue()));

			if (UMLViewsRepository.Usage.visibility == event.getAffectedEditor())
				command.append(SetCommand.create(liveEditingDomain, usage, UMLPackage.eINSTANCE
						.getNamedElement_Visibility(), event.getNewValue()));

			if (UMLViewsRepository.Usage.supplier == event.getAffectedEditor()) {
				if (PropertiesEditionEvent.ADD == event.getKind())
					command.append(AddCommand.create(liveEditingDomain, usage, UMLPackage.eINSTANCE
							.getDependency_Supplier(), event.getNewValue()));
				if (PropertiesEditionEvent.REMOVE == event.getKind())
					command.append(RemoveCommand.create(liveEditingDomain, usage, UMLPackage.eINSTANCE
							.getDependency_Supplier(), event.getNewValue()));
				if (PropertiesEditionEvent.MOVE == event.getKind())
					command.append(MoveCommand.create(liveEditingDomain, usage, UMLPackage.eINSTANCE
