...
public class ReadIsClassifiedObjectActionBasePropertiesEditionComponent extends StandardPropertiesEditionComponent {
	 ...
	 * Default constructor
	 */
	public ReadIsClassifiedObjectActionBasePropertiesEditionComponent(EObject readIsClassifiedObjectAction, String editing_mode) {
		...
		if (PropertiesEditionEvent.COMMIT == event.getState() && IPropertiesEditionComponent.LIVE_MODE.equals(editing_mode)) {
			...
			if (UMLViewsRepository.ReadIsClassifiedObjectAction.clientDependency == event.getAffectedEditor()) {
				...
				if (PropertiesEditionEvent.MOVE == event.getKind())
					command.append(MoveCommand.create(liveEditingDomain, readIsClassifiedObjectAction, UMLPackage.eINSTANCE.getNamedElement_ClientDependency(), event.getNewValue(), event
							.getNewIndex()));
			}
			...
			if (UMLViewsRepository.ReadIsClassifiedObjectAction.inInterruptibleRegion == event.getAffectedEditor()) {
				...
				if (PropertiesEditionEvent.MOVE == event.getKind())
					command.append(MoveCommand.create(liveEditingDomain, readIsClassifiedObjectAction, UMLPackage.eINSTANCE.getActivityNode_InInterruptibleRegion(), event.getNewValue(), event
							.getNewIndex()));
			}
