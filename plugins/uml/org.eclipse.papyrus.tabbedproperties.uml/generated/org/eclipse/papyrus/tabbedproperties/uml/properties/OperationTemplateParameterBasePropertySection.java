...
import org.eclipse.papyrus.tabbedproperties.uml.components.OperationTemplateParameterBasePropertiesEditionComponent;
 ...
 */
public class OperationTemplateParameterBasePropertySection extends AbstractPropertySection implements IFilter {
		...
		if (editingDomain != null && newEObject != null && newEObject != eObject) {
			...
			if (eObject != null) {
				...
				this.propertiesEditionComponent = provider.getPropertiesEditionComponent(eObject, IPropertiesEditionComponent.LIVE_MODE,
						OperationTemplateParameterBasePropertiesEditionComponent.BASE_PART);
				if (this.propertiesEditionComponent != null) {
					this.propertiesEditionComponent.setLiveEditingDomain(editingDomain);
					// FIXME: find a better way to define the Form constant
					this.editionPart = propertiesEditionComponent.getPropertiesEditionPart(1, OperationTemplateParameterBasePropertiesEditionComponent.BASE_PART); //$NON-NLS-1$
					if (editionPart instanceof IFormPropertiesEditionPart) {
						...
						parent.layout();
						propertiesEditionComponent.initPart(propertiesEditionComponent.translatePart(OperationTemplateParameterBasePropertiesEditionComponent.BASE_PART), 1, eObject);
					}
				}
			}
		}
	...
	public boolean select(Object toTest) {
		...
		return eObjectToTest != null && eObjectToTest.eClass() == UMLPackage.eINSTANCE.getOperationTemplateParameter();
	}
