		...
		if (PropertiesEditionEvent.COMMIT == event.getState() && IPropertiesEditionComponent.LIVE_MODE.equals(editing_mode)) {
			...
			if (UMLViewsRepository.ActionExecutionSpecification.clientDependency == event.getAffectedEditor()) {
				...
				if (PropertiesEditionEvent.MOVE == event.getKind())
					command.append(MoveCommand.create(liveEditingDomain, actionExecutionSpecification, UMLPackage.eINSTANCE.getNamedElement_ClientDependency(), event.getNewValue(), event
							.getNewIndex()));
			}
			if (UMLViewsRepository.ActionExecutionSpecification.covered == event.getAffectedEditor()) {
				...
				if (PropertiesEditionEvent.MOVE == event.getKind())
					command
							.append(MoveCommand
									.create(liveEditingDomain, actionExecutionSpecification, UMLPackage.eINSTANCE.getInteractionFragment_Covered(), event.getNewValue(), event.getNewIndex()));
			}
