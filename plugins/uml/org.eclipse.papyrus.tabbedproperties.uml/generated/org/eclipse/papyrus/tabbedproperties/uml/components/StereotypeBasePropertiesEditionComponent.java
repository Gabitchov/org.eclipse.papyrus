...
public class StereotypeBasePropertiesEditionComponent extends StandardPropertiesEditionComponent {
	...
	private AdapterImpl initializeSemanticAdapter() {
		return new EContentAdapter() {
			...
			public void notifyChanged(Notification msg) {
				...
				if (msg.getFeature() != null
						&& (((EStructuralFeature) msg.getFeature()) == UMLPackage.eINSTANCE.getStereotype_Icon() || ((EStructuralFeature) msg.getFeature()).getEContainingClass() == UMLPackage.eINSTANCE
								.getImage())) {
	...
	public CompoundCommand getPropertiesEditionCommand(EditingDomain editingDomain) {
		...
		if (stereotype != null) {
			...
			List iconToAdd = basePart.getIconToAdd();
			for (Iterator iter = iconToAdd.iterator(); iter.hasNext();)
				cc.append(AddCommand.create(editingDomain, stereotype, UMLPackage.eINSTANCE.getStereotype_Icon(), iter.next()));
			Map iconToRefresh = basePart.getIconToEdit();
			for (Iterator iter = iconToRefresh.keySet().iterator(); iter.hasNext();) {

				// Start of user code for icon reference refreshment

				Image nextElement = (Image) iter.next();
				Image icon = (Image) iconToRefresh.get(nextElement);

				// End of user code
			}
			List iconToRemove = basePart.getIconToRemove();
			for (Iterator iter = iconToRemove.iterator(); iter.hasNext();)
				cc.append(DeleteCommand.create(editingDomain, iter.next()));
			List iconToMove = basePart.getIconToMove();
			for (Iterator iter = iconToMove.iterator(); iter.hasNext();) {
				org.eclipse.emf.eef.runtime.impl.utils.EMFListEditUtil.MoveElement moveElement = (org.eclipse.emf.eef.runtime.impl.utils.EMFListEditUtil.MoveElement) iter.next();
	...
	public String getHelpContent(String key, int kind) {
		...
		if (key == UMLViewsRepository.Stereotype.icon)
			return "Stereotype can change the graphical appearance of the extended model element by using attached icons. When this association is not null, it references the location of the icon content to be displayed within diagrams presenting the extended model elements."; //$NON-NLS-1$
