...
public class DecisionNodeBasePropertiesEditionComponent extends StandardPropertiesEditionComponent {
	...
	public IPropertiesEditionPart getPropertiesEditionPart(int kind, String key) {
		if (decisionNode != null && BASE_PART.equals(key)) {
			if (basePart == null) {
				...
				if (provider != null) {
					basePart = (DecisionNodePropertiesEditionPart) provider.getPropertiesEditionPart(
							UMLViewsRepository.DecisionNode.class, kind, this);
	...
	public void initPart(java.lang.Class key, int kind, EObject elt, ResourceSet allResource) {
		if (basePart != null && key == UMLViewsRepository.DecisionNode.class) {
			...
			DecisionNode decisionNode = (DecisionNode) elt;
			// init values
			...
			basePart.initVisibility((EEnum) UMLPackage.eINSTANCE.getNamedElement_Visibility().getEType(), decisionNode
					.getVisibility());
			...
			basePart.initInInterruptibleRegion(decisionNode, null, UMLPackage.eINSTANCE
	...
	public CompoundCommand getPropertiesEditionCommand(EditingDomain editingDomain) {
		...
		if (decisionNode != null) {
			...
			for (Iterator iter = ownedCommentToAddFromOwnedComment.iterator(); iter.hasNext();)
				cc.append(AddCommand.create(editingDomain, decisionNode,
			...
			for (Iterator iter = ownedCommentToMoveFromOwnedComment.iterator(); iter.hasNext();) {
				org.eclipse.emf.eef.runtime.impl.utils.EMFListEditUtil.MoveElement moveElement = (org.eclipse.emf.eef.runtime.impl.utils.EMFListEditUtil.MoveElement) iter
						.next();
				cc.append(MoveCommand.create(editingDomain, decisionNode, UMLPackage.eINSTANCE.getComment(),
						moveElement.getElement(), moveElement.getIndex()));
			}
			cc.append(SetCommand.create(editingDomain, decisionNode, UMLPackage.eINSTANCE.getNamedElement_Name(),
					basePart.getName()));

			cc.append(SetCommand.create(editingDomain, decisionNode, UMLPackage.eINSTANCE.getNamedElement_Visibility(),
					basePart.getVisibility()));

			List clientDependencyToAddFromClientDependency = basePart.getClientDependencyToAdd();
			for (Iterator iter = clientDependencyToAddFromClientDependency.iterator(); iter.hasNext();)
				cc.append(AddCommand.create(editingDomain, decisionNode, UMLPackage.eINSTANCE
						.getNamedElement_ClientDependency(), iter.next()));
			List clientDependencyToRemoveFromClientDependency = basePart.getClientDependencyToRemove();
			for (Iterator iter = clientDependencyToRemoveFromClientDependency.iterator(); iter.hasNext();)
				cc.append(RemoveCommand.create(editingDomain, decisionNode, UMLPackage.eINSTANCE
						.getNamedElement_ClientDependency(), iter.next()));
			...
			// cc.append(MoveCommand.create(editingDomain, decisionNode,
			// UMLPackage.eINSTANCE.getDependency(), moveElement.getElement(),
			// moveElement.getIndex()));
			// }
			cc.append(SetCommand.create(editingDomain, decisionNode, UMLPackage.eINSTANCE
		...
		if (PropertiesEditionEvent.COMMIT == event.getState()
			...
			if (UMLViewsRepository.DecisionNode.ownedComment == event.getAffectedEditor()) {
				if (PropertiesEditionEvent.SET == event.getKind()) {
					...
					command.append(AddCommand.create(liveEditingDomain, decisionNode, UMLPackage.eINSTANCE
							.getElement_OwnedComment(), event.getNewValue()));
				else if (PropertiesEditionEvent.REMOVE == event.getKind())
					command.append(DeleteCommand.create(liveEditingDomain, event.getNewValue()));
				else if (PropertiesEditionEvent.MOVE == event.getKind())
					command.append(MoveCommand.create(liveEditingDomain, decisionNode, UMLPackage.eINSTANCE
							.getComment(), event.getNewValue(), event.getNewIndex()));
			}
			if (UMLViewsRepository.DecisionNode.name == event.getAffectedEditor())
				command.append(SetCommand.create(liveEditingDomain, decisionNode, UMLPackage.eINSTANCE
			...
			if (UMLViewsRepository.DecisionNode.clientDependency == event.getAffectedEditor()) {
				...
				if (PropertiesEditionEvent.REMOVE == event.getKind())
					command.append(RemoveCommand.create(liveEditingDomain, decisionNode, UMLPackage.eINSTANCE
							.getNamedElement_ClientDependency(), event.getNewValue()));
	...
	public String getHelpContent(String key, int kind) {
		if (key == UMLViewsRepository.DecisionNode.ownedComment)
			return null; //$NON-NLS-1$
		if (key == UMLViewsRepository.DecisionNode.name)
			return null; //$NON-NLS-1$
		if (key == UMLViewsRepository.DecisionNode.visibility)
			return null; //$NON-NLS-1$
		if (key == UMLViewsRepository.DecisionNode.clientDependency)
			return null; //$NON-NLS-1$
		if (key == UMLViewsRepository.DecisionNode.isLeaf)
			return null; //$NON-NLS-1$
		if (key == UMLViewsRepository.DecisionNode.outgoing)
			return null; //$NON-NLS-1$
		if (key == UMLViewsRepository.DecisionNode.incoming)
			return null; //$NON-NLS-1$
		if (key == UMLViewsRepository.DecisionNode.inPartition)
			return null; //$NON-NLS-1$
		if (key == UMLViewsRepository.DecisionNode.inInterruptibleRegion)
			return null; //$NON-NLS-1$
		if (key == UMLViewsRepository.DecisionNode.redefinedNode)
			return null; //$NON-NLS-1$
	...
	public Diagnostic validate() {
		if (IPropertiesEditionComponent.BATCH_MODE.equals(editing_mode)) {
			...
			return Diagnostician.INSTANCE.validate(decisionNode);
		else
			return null;
	}
