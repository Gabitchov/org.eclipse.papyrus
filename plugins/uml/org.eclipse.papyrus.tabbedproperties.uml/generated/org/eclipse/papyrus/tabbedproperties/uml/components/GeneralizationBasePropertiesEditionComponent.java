...
public class GeneralizationBasePropertiesEditionComponent extends StandardPropertiesEditionComponent {
	...
	public IPropertiesEditionPart getPropertiesEditionPart(int kind, String key) {
		if (generalization != null && BASE_PART.equals(key)) {
			if (basePart == null) {
				...
				if (provider != null) {
					basePart = (GeneralizationPropertiesEditionPart) provider.getPropertiesEditionPart(
							UMLViewsRepository.Generalization.class, kind, this);
	...
	public void initPart(java.lang.Class key, int kind, EObject elt, ResourceSet allResource) {
		if (basePart != null && key == UMLViewsRepository.Generalization.class) {
			...
			basePart.initGeneralizationSet(generalization, null, UMLPackage.eINSTANCE
	...
	public CompoundCommand getPropertiesEditionCommand(EditingDomain editingDomain) {
		...
		if (generalization != null) {
			cc.append(SetCommand.create(editingDomain, generalization, UMLPackage.eINSTANCE
					.getGeneralization_IsSubstitutable(), basePart.getIsSubstitutable()));
			...
			for (Iterator iter = generalizationSetToAddFromGeneralizationSet.iterator(); iter.hasNext();)
				cc.append(AddCommand.create(editingDomain, generalization, UMLPackage.eINSTANCE
						.getGeneralization_GeneralizationSet(), iter.next()));
			...
			for (Iterator iter = generalizationSetToRemoveFromGeneralizationSet.iterator(); iter.hasNext();)
				cc.append(RemoveCommand.create(editingDomain, generalization, UMLPackage.eINSTANCE
						.getGeneralization_GeneralizationSet(), iter.next()));
			// List generalizationSetToMoveFromGeneralizationSet =
			// basePart.getGeneralizationSetToMove();
			// for (Iterator iter = generalizationSetToMoveFromGeneralizationSet.iterator();
			// iter.hasNext();){
			...
			// cc.append(MoveCommand.create(editingDomain, generalization,
	...
	public void firePropertiesChanged(PropertiesEditionEvent event) {
		...
		if (PropertiesEditionEvent.COMMIT == event.getState()
			...
			if (UMLViewsRepository.Generalization.isSubstitutable == event.getAffectedEditor())
				command.append(SetCommand.create(liveEditingDomain, generalization, UMLPackage.eINSTANCE
						.getGeneralization_IsSubstitutable(), event.getNewValue()));

			if (UMLViewsRepository.Generalization.generalizationSet == event.getAffectedEditor()) {
				if (PropertiesEditionEvent.ADD == event.getKind())
					command.append(AddCommand.create(liveEditingDomain, generalization, UMLPackage.eINSTANCE
							.getGeneralization_GeneralizationSet(), event.getNewValue()));
				if (PropertiesEditionEvent.REMOVE == event.getKind())
					command.append(RemoveCommand.create(liveEditingDomain, generalization, UMLPackage.eINSTANCE
							.getGeneralization_GeneralizationSet(), event.getNewValue()));
				if (PropertiesEditionEvent.MOVE == event.getKind())
					command.append(MoveCommand.create(liveEditingDomain, generalization, UMLPackage.eINSTANCE
							.getGeneralization_GeneralizationSet(), event.getNewValue(), event.getNewIndex()));
			}
	...
	public Diagnostic validateValue(PropertiesEditionEvent event) {
		...
		try {
			if (UMLViewsRepository.Generalization.isSubstitutable == event.getAffectedEditor()) {
				Object newValue = EcoreUtil.createFromString(UMLPackage.eINSTANCE.getGeneralization_IsSubstitutable()
						.getEAttributeType(), newStringValue);
				ret = Diagnostician.INSTANCE.validate(UMLPackage.eINSTANCE.getGeneralization_IsSubstitutable()
