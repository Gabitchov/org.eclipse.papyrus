	...
	public void setInput(IWorkbenchPart part, ISelection selection) {
		...
		if (editingDomain != null && newEObject != null && newEObject != eObject) {
			...
			if (eObject != null) {
				...
				this.propertiesEditionComponent = provider.getPropertiesEditionComponent(eObject, IPropertiesEditionComponent.LIVE_MODE, IncludeBasePropertiesEditionComponent.BASE_PART);
				if (this.propertiesEditionComponent != null) {
					this.propertiesEditionComponent.setLiveEditingDomain(editingDomain);
					// FIXME: find a better way to define the Form constant
					this.editionPart = propertiesEditionComponent.getPropertiesEditionPart(1, IncludeBasePropertiesEditionComponent.BASE_PART); //$NON-NLS-1$
					if (editionPart instanceof IFormPropertiesEditionPart) {
						...
						parent.layout();
						propertiesEditionComponent.initPart(propertiesEditionComponent.translatePart(IncludeBasePropertiesEditionComponent.BASE_PART), 1, eObject);
					}
				}
			}
		}
