...
import org.eclipse.papyrus.tabbedproperties.uml.figures.SlotFormForm;
...
/**
 * The Slot property section for the Slot element
 * 
 ...
 * @generated
 */
public class SlotSection extends AbstractEnhancedPropertySection implements IFilter {

	/**
	 * The section's form
	 * @generated 
	 */
	private SlotFormForm slotForm;
	...
	public PropertyEditor createFigure() {
		slotForm = new SlotFormForm("","A slot specifies that an entity modeled by an instance specification has a value or values for a specific structural feature.");
		slotForm.setBackContext(backbone);
		return slotForm;
	}
	...
	{
				//---------------------------------------------------------------------
		//mapping defining feature
		//---------------------------------------------------------------------
				
			 
		//listener for a simple feature definingFeature of Slot
		PropertyChangeListener<StructuralFeature> definingFeatureListener = new PropertyChangeListener<StructuralFeature>(){
		   public void propertyChanged(PropertyEditor editor, StructuralFeature newValue) {
				SetValueCommand command = new SetValueCommand(editingDomain, slotElement, UMLPackage.eINSTANCE.getSlot_DefiningFeature(), newValue);
			editingDomain.getCommandStack().execute(command);
			}
		};

		slotForm.getDefiningFeature().setPropertyChangeListener(definingFeatureListener);
		//create listener to create directly element
		CreateElementListener definingFeatureCreateListener= new CreateElementListener(){
			public void mouseUp(MouseEvent e) {
			//
				CreateElementCommand command= new CreateElementCommand(editingDomain,slotElement,UMLPackage.eINSTANCE.getSlot_DefiningFeature(),UMLPackage.eINSTANCE.getStructuralFeature());
				editingDomain.getCommandStack().execute(command);
			}
		};
		//this is not a container or it is abstract
		slotForm.getDefiningFeature().setCreateElement(definingFeatureCreateListener);
		slotForm.getDefiningFeature().setAbstractElement(true);
		//set the navigation service
		slotForm.getDefiningFeature().setNavigationListener(navigationListener);
	...
	public void refresh() {
		...
		if(slotElement.getDefiningFeature()==null){
		    slotForm.getDefiningFeature().setValue("<UNDEFINED>");}
		else{
			slotForm.getDefiningFeature().setValue(slotElement.getDefiningFeature().getQualifiedName());
			navigationListener.setTheElement(slotElement.getDefiningFeature());
			}
	...
	public void setInput(IWorkbenchPart part, ISelection selection) {
    	...
    	this.slotElement = (Slot)getBusinessElement(input);
    	
		 // setup the backbone for referencedElementForm
		slotForm.getDefiningFeature().setBackboneContext(backbone);
	}
	...
	public boolean select(Object toTest) {
		return getBusinessElement(toTest) instanceof Slot;
	}
	...
	public void aboutToBeShown() {
		...
		// Listen to the model
		slotElement.eAdapters().add(modelListener);
	}
	...
	public void aboutToBeHidden() {
		// Stop listening
		slotElement.eAdapters().remove(modelListener);
