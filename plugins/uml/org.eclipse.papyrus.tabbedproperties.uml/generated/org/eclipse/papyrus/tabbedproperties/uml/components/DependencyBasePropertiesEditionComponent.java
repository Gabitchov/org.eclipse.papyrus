 ...
 */
public class DependencyBasePropertiesEditionComponent extends StandardPropertiesEditionComponent {
	 ...
	 * The Base part
	 */
	private DependencyPropertiesEditionPart basePart;
	...
	public IPropertiesEditionPart getPropertiesEditionPart(int kind, String key) {
		if (dependency != null && BASE_PART.equals(key)) {
			if (basePart == null) {
				...
				if (provider != null) {
					basePart = (DependencyPropertiesEditionPart) provider.getPropertiesEditionPart(
							UMLViewsRepository.Dependency.class, kind, this);
	...
	public void initPart(java.lang.Class key, int kind, EObject elt, ResourceSet allResource) {
		if (basePart != null && key == UMLViewsRepository.Dependency.class) {
			...
			Dependency dependency = (Dependency) elt;
			// init values
			if (dependency.getName() != null)
				basePart.setName(dependency.getName());

			basePart.initVisibility((EEnum) UMLPackage.eINSTANCE.getNamedElement_Visibility().getEType(), dependency
					.getVisibility());
	...
	public CompoundCommand getPropertiesEditionCommand(EditingDomain editingDomain) {
		...
		if (dependency != null) {
			cc.append(SetCommand.create(editingDomain, dependency, UMLPackage.eINSTANCE.getNamedElement_Name(),
					basePart.getName()));

			cc.append(SetCommand.create(editingDomain, dependency, UMLPackage.eINSTANCE.getNamedElement_Visibility(),
			...
			for (Iterator iter = supplierToAddFromSupplier.iterator(); iter.hasNext();)
				cc.append(AddCommand.create(editingDomain, dependency, UMLPackage.eINSTANCE.getDependency_Supplier(),
						iter.next()));
			...
			for (Iterator iter = supplierToRemoveFromSupplier.iterator(); iter.hasNext();)
				cc.append(RemoveCommand.create(editingDomain, dependency,
			...
			// cc.append(MoveCommand.create(editingDomain, dependency,
			...
			for (Iterator iter = clientToAddFromClient.iterator(); iter.hasNext();)
				cc.append(AddCommand.create(editingDomain, dependency, UMLPackage.eINSTANCE.getDependency_Client(),
						iter.next()));
			List clientToRemoveFromClient = basePart.getClientToRemove();
			for (Iterator iter = clientToRemoveFromClient.iterator(); iter.hasNext();)
				cc.append(RemoveCommand.create(editingDomain, dependency, UMLPackage.eINSTANCE.getDependency_Client(),
						iter.next()));
	...
	public EObject getPropertiesEditionObject(EObject source) {
		if (source instanceof Dependency) {
			...
			dependencyToUpdate.setVisibility((VisibilityKind) basePart.getVisibility());

			dependencyToUpdate.getSuppliers().addAll(basePart.getSupplierToAdd());
			dependencyToUpdate.getClients().addAll(basePart.getClientToAdd());
	...
	public void firePropertiesChanged(PropertiesEditionEvent event) {
		...
		if (PropertiesEditionEvent.COMMIT == event.getState()
			...
			if (UMLViewsRepository.Dependency.name == event.getAffectedEditor())
				command.append(SetCommand.create(liveEditingDomain, dependency, UMLPackage.eINSTANCE
			...
			if (UMLViewsRepository.Dependency.supplier == event.getAffectedEditor()) {
				if (PropertiesEditionEvent.ADD == event.getKind())
					command.append(AddCommand.create(liveEditingDomain, dependency, UMLPackage.eINSTANCE
							.getDependency_Supplier(), event.getNewValue()));
				if (PropertiesEditionEvent.REMOVE == event.getKind())
					command.append(RemoveCommand.create(liveEditingDomain, dependency, UMLPackage.eINSTANCE
							.getDependency_Supplier(), event.getNewValue()));
				if (PropertiesEditionEvent.MOVE == event.getKind())
					command.append(MoveCommand.create(liveEditingDomain, dependency, UMLPackage.eINSTANCE
	...
	public boolean isRequired(String key, int kind) {
		return key == UMLViewsRepository.Dependency.supplier || key == UMLViewsRepository.Dependency.client;
	}
	...
	public String getHelpContent(String key, int kind) {
		if (key == UMLViewsRepository.Dependency.name)
			return null; //$NON-NLS-1$
		if (key == UMLViewsRepository.Dependency.visibility)
			return null; //$NON-NLS-1$
		if (key == UMLViewsRepository.Dependency.supplier)
			return null; //$NON-NLS-1$
		if (key == UMLViewsRepository.Dependency.client)
			return null; //$NON-NLS-1$
	...
	public Diagnostic validate() {
		if (IPropertiesEditionComponent.BATCH_MODE.equals(editing_mode)) {
			...
			return Diagnostician.INSTANCE.validate(dependency);
		else
			return null;
	}
