	...
	public void firePropertiesChanged(PropertiesEditionEvent event) {
		...
		if (PropertiesEditionEvent.COMMIT == event.getState() && IPropertiesEditionComponent.LIVE_MODE.equals(editing_mode)) {
			...
			if (UMLViewsRepository.StructuredActivityNode.inInterruptibleRegion == event.getAffectedEditor()) {
				...
				if (PropertiesEditionEvent.MOVE == event.getKind())
					command.append(MoveCommand.create(liveEditingDomain, structuredActivityNode, UMLPackage.eINSTANCE.getActivityNode_InInterruptibleRegion(), event.getNewValue(), event.getNewIndex()));
			}
