...
public class DeploymentBasePropertiesEditionComponent extends StandardPropertiesEditionComponent {
		...
		return new EContentAdapter() {
			...
			public void notifyChanged(Notification msg) {
				if (basePart == null)
					...
					if (msg.getFeature() != null && 
							(((EStructuralFeature)msg.getFeature()) == UMLPackage.eINSTANCE.getDeployment_Configuration()
							|| ((EStructuralFeature)msg.getFeature()).getEContainingClass() == UMLPackage.eINSTANCE.getDeployment_Configuration())) {
	...
	public CompoundCommand getPropertiesEditionCommand(EditingDomain editingDomain) {
		...
		if (deployment != null) {
			...
			//List deployedArtifactToMoveFromDeployedArtifact = basePart.getDeployedArtifactToMove();
			//for (Iterator iter = deployedArtifactToMoveFromDeployedArtifact.iterator(); iter.hasNext();){
			//	org.eclipse.emf.eef.runtime.impl.utils.EMFListEditUtil.MoveElement moveElement = (org.eclipse.emf.eef.runtime.impl.utils.EMFListEditUtil.MoveElement)iter.next();
			...
			for (Iterator iter = configurationToRefreshFromConfiguration.keySet().iterator(); iter.hasNext();) {
				...
				DeploymentSpecification configuration = (DeploymentSpecification) configurationToRefreshFromConfiguration.get(nextElement);
				
				for (EStructuralFeature feature : nextElement.eClass().getEAllStructuralFeatures()) {
					if (feature.isChangeable() && !(feature instanceof EReference && ((EReference) feature).isContainer())) {
						cc.append(SetCommand.create(editingDomain, nextElement, feature, configuration.eGet(feature)));
					}
				}
	...
	public Diagnostic validate() {
		...
		if (IPropertiesEditionComponent.BATCH_MODE.equals(editing_mode)) {
			...
			validate = Diagnostician.INSTANCE.validate(deployment);
