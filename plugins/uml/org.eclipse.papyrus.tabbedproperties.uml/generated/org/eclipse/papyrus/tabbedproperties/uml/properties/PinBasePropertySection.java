...
import org.eclipse.papyrus.tabbedproperties.uml.components.PinBasePropertiesEditionComponent;

/**
 * @author <a href="mailto:jerome.benois@obeo.fr">Jerome Benois</a>
 */
public class PinBasePropertySection extends AbstractPropertySection implements IFilter {
	...
	public void setInput(IWorkbenchPart part, ISelection selection) {
		...
		if (editingDomain != null && newEObject != null && newEObject != eObject) {
			...
			if (eObject != null) {
				...
				this.propertiesEditionComponent = provider.getPropertiesEditionComponent(eObject, IPropertiesEditionComponent.LIVE_MODE, PinBasePropertiesEditionComponent.BASE_PART);
				if (this.propertiesEditionComponent != null) {
					this.propertiesEditionComponent.setLiveEditingDomain(editingDomain);
					// FIXME: find a better way to define the Form constant
					this.editionPart = propertiesEditionComponent.getPropertiesEditionPart(1, PinBasePropertiesEditionComponent.BASE_PART); //$NON-NLS-1$
					if (editionPart instanceof IFormPropertiesEditionPart) {
						...
						parent.layout();
						propertiesEditionComponent.initPart(propertiesEditionComponent.translatePart(PinBasePropertiesEditionComponent.BASE_PART), 1, eObject);
					}
				}
			}
		}
	...
	public boolean select(Object toTest) {
		...
		return eObjectToTest != null && eObjectToTest.eClass() == UMLPackage.eINSTANCE.getPin();
	}
