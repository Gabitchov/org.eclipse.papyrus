 ...
 */
public class SlotBasePropertiesEditionComponent extends StandardPropertiesEditionComponent {
		...
		return new EContentAdapter() {
			...
			public void notifyChanged(Notification msg) {
				if (msg.getFeature() != null
						&& (((EStructuralFeature) msg.getFeature()) == UMLPackage.eINSTANCE.getSlot_Value() || ((EStructuralFeature) msg.getFeature()).getEContainingClass() == UMLPackage.eINSTANCE
	...
	public java.lang.Class translatePart(String key) {
		if (BASE_PART.equals(key))
			return UMLViewsRepository.Slot.class;
	...
	public CompoundCommand getPropertiesEditionCommand(EditingDomain editingDomain) {
		...
		if (slot != null) {
			List valueToAdd = basePart.getValueToAdd();
			for (Iterator iter = valueToAdd.iterator(); iter.hasNext();)
				cc.append(AddCommand.create(editingDomain, slot, UMLPackage.eINSTANCE.getSlot_Value(), iter.next()));
			Map valueToRefresh = basePart.getValueToEdit();
			for (Iterator iter = valueToRefresh.keySet().iterator(); iter.hasNext();) {

				// Start of user code for value reference refreshment

				ValueSpecification nextElement = (ValueSpecification) iter.next();
				ValueSpecification value = (ValueSpecification) valueToRefresh.get(nextElement);

				// End of user code
			}
			List valueToRemove = basePart.getValueToRemove();
			for (Iterator iter = valueToRemove.iterator(); iter.hasNext();)
				cc.append(DeleteCommand.create(editingDomain, iter.next()));
			List valueToMove = basePart.getValueToMove();
			for (Iterator iter = valueToMove.iterator(); iter.hasNext();) {
	...
	public String getHelpContent(String key, int kind) {
		if (key == UMLViewsRepository.Slot.value)
			return "The value or values corresponding to the defining feature for the owning instance specification."; //$NON-NLS-1$
	...
	public void dispose() {
		if (semanticAdapter != null)
			slot.eAdapters().remove(semanticAdapter);
	}
