...
public class InterruptibleActivityRegionBasePropertiesEditionComponent extends StandardPropertiesEditionComponent {
	 ...
	 * Default constructor
	 */
	public InterruptibleActivityRegionBasePropertiesEditionComponent(EObject interruptibleActivityRegion,
			String editing_mode) {
	...
	private AdapterImpl initializeSemanticAdapter() {
		return new EContentAdapter() {
			...
			public void notifyChanged(Notification msg) {
				if (basePart == null)
					InterruptibleActivityRegionBasePropertiesEditionComponent.this.dispose();
				else {
	...
	public IPropertiesEditionPart getPropertiesEditionPart(int kind, String key) {
		if (interruptibleActivityRegion != null && BASE_PART.equals(key)) {
			if (basePart == null) {
				...
				if (provider != null) {
					basePart = (InterruptibleActivityRegionPropertiesEditionPart) provider.getPropertiesEditionPart(
							UMLViewsRepository.InterruptibleActivityRegion.class, kind, this);
	...
	public void setPropertiesEditionPart(java.lang.Class key, int kind, IPropertiesEditionPart propertiesEditionPart) {
		if (key == UMLViewsRepository.InterruptibleActivityRegion.class)
			this.basePart = (InterruptibleActivityRegionPropertiesEditionPart) propertiesEditionPart;
	}
	...
	public void initPart(java.lang.Class key, int kind, EObject elt, ResourceSet allResource) {
		if (basePart != null && key == UMLViewsRepository.InterruptibleActivityRegion.class) {
			...
			// init values
			basePart
					.initOwnedComment(interruptibleActivityRegion, null, UMLPackage.eINSTANCE.getElement_OwnedComment());
			basePart.initNode(interruptibleActivityRegion, null, UMLPackage.eINSTANCE
					.getInterruptibleActivityRegion_Node());
			basePart.initInterruptingEdge(interruptibleActivityRegion, null, UMLPackage.eINSTANCE
			...
			basePart.addFilterToInterruptingEdge(new ViewerFilter() {
				...
				public boolean select(Viewer viewer, Object parentElement, Object element) {
					if (element instanceof EObject)
						return (!basePart.getInterruptingEdgeTable().contains(element));
					return false;
				}
	...
	public CompoundCommand getPropertiesEditionCommand(EditingDomain editingDomain) {
		...
		if (interruptibleActivityRegion != null) {
			...
			for (Iterator iter = ownedCommentToAddFromOwnedComment.iterator(); iter.hasNext();)
				cc.append(AddCommand.create(editingDomain, interruptibleActivityRegion, UMLPackage.eINSTANCE
			...
			for (Iterator iter = ownedCommentToMoveFromOwnedComment.iterator(); iter.hasNext();) {
				org.eclipse.emf.eef.runtime.impl.utils.EMFListEditUtil.MoveElement moveElement = (org.eclipse.emf.eef.runtime.impl.utils.EMFListEditUtil.MoveElement) iter
						.next();
				cc.append(MoveCommand.create(editingDomain, interruptibleActivityRegion, UMLPackage.eINSTANCE
			...
			for (Iterator iter = nodeToAddFromNode.iterator(); iter.hasNext();)
				cc.append(AddCommand.create(editingDomain, interruptibleActivityRegion, UMLPackage.eINSTANCE
						.getInterruptibleActivityRegion_Node(), iter.next()));
			List nodeToRemoveFromNode = basePart.getNodeToRemove();
			for (Iterator iter = nodeToRemoveFromNode.iterator(); iter.hasNext();)
				cc.append(RemoveCommand.create(editingDomain, interruptibleActivityRegion, UMLPackage.eINSTANCE
						.getInterruptibleActivityRegion_Node(), iter.next()));
			...
			// cc.append(MoveCommand.create(editingDomain, interruptibleActivityRegion,
			// UMLPackage.eINSTANCE.getActivityNode(), moveElement.getElement(),
			// moveElement.getIndex()));
			// }
			List interruptingEdgeToAddFromInterruptingEdge = basePart.getInterruptingEdgeToAdd();
			for (Iterator iter = interruptingEdgeToAddFromInterruptingEdge.iterator(); iter.hasNext();)
				cc.append(AddCommand.create(editingDomain, interruptibleActivityRegion, UMLPackage.eINSTANCE
						.getInterruptibleActivityRegion_InterruptingEdge(), iter.next()));
			List interruptingEdgeToRemoveFromInterruptingEdge = basePart.getInterruptingEdgeToRemove();
			for (Iterator iter = interruptingEdgeToRemoveFromInterruptingEdge.iterator(); iter.hasNext();)
				cc.append(RemoveCommand.create(editingDomain, interruptibleActivityRegion, UMLPackage.eINSTANCE
						.getInterruptibleActivityRegion_InterruptingEdge(), iter.next()));
			// List interruptingEdgeToMoveFromInterruptingEdge =
			// basePart.getInterruptingEdgeToMove();
			// for (Iterator iter = interruptingEdgeToMoveFromInterruptingEdge.iterator();
			// iter.hasNext();){
			// org.eclipse.emf.eef.runtime.impl.utils.EMFListEditUtil.MoveElement moveElement =
			// (org.eclipse.emf.eef.runtime.impl.utils.EMFListEditUtil.MoveElement)iter.next();
			// cc.append(MoveCommand.create(editingDomain, interruptibleActivityRegion,
		...
		if (PropertiesEditionEvent.COMMIT == event.getState()
			...
			if (UMLViewsRepository.InterruptibleActivityRegion.ownedComment == event.getAffectedEditor()) {
				if (PropertiesEditionEvent.SET == event.getKind()) {
					...
					command.append(AddCommand.create(liveEditingDomain, interruptibleActivityRegion,
							UMLPackage.eINSTANCE.getElement_OwnedComment(), event.getNewValue()));
				else if (PropertiesEditionEvent.REMOVE == event.getKind())
					command.append(DeleteCommand.create(liveEditingDomain, event.getNewValue()));
				else if (PropertiesEditionEvent.MOVE == event.getKind())
					command.append(MoveCommand.create(liveEditingDomain, interruptibleActivityRegion,
			...
			if (UMLViewsRepository.InterruptibleActivityRegion.node == event.getAffectedEditor()) {
				if (PropertiesEditionEvent.ADD == event.getKind())
					command.append(AddCommand.create(liveEditingDomain, interruptibleActivityRegion,
							UMLPackage.eINSTANCE.getInterruptibleActivityRegion_Node(), event.getNewValue()));
				if (PropertiesEditionEvent.REMOVE == event.getKind())
					command.append(RemoveCommand.create(liveEditingDomain, interruptibleActivityRegion,
							UMLPackage.eINSTANCE.getInterruptibleActivityRegion_Node(), event.getNewValue()));
				if (PropertiesEditionEvent.MOVE == event.getKind())
					command.append(MoveCommand.create(liveEditingDomain, interruptibleActivityRegion,
							UMLPackage.eINSTANCE.getInterruptibleActivityRegion_Node(), event.getNewValue(), event
									.getNewIndex()));
			}
			if (UMLViewsRepository.InterruptibleActivityRegion.interruptingEdge == event.getAffectedEditor()) {
				if (PropertiesEditionEvent.ADD == event.getKind())
					command.append(AddCommand
							.create(liveEditingDomain, interruptibleActivityRegion, UMLPackage.eINSTANCE
									.getInterruptibleActivityRegion_InterruptingEdge(), event.getNewValue()));
				if (PropertiesEditionEvent.REMOVE == event.getKind())
