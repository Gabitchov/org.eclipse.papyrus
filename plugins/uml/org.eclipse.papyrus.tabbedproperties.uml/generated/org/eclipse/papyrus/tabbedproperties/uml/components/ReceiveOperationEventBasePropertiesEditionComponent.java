...
public class ReceiveOperationEventBasePropertiesEditionComponent extends StandardPropertiesEditionComponent {
	 ...
	 * Default constructor
	 */
	public ReceiveOperationEventBasePropertiesEditionComponent(EObject receiveOperationEvent, String editing_mode) {
	...
	private AdapterImpl initializeSemanticAdapter() {
		return new EContentAdapter() {
			...
			public void notifyChanged(Notification msg) {
				if (basePart == null)
					ReceiveOperationEventBasePropertiesEditionComponent.this.dispose();
				else {
	...
	public IPropertiesEditionPart getPropertiesEditionPart(int kind, String key) {
		if (receiveOperationEvent != null && BASE_PART.equals(key)) {
			if (basePart == null) {
				...
				if (provider != null) {
					basePart = (ReceiveOperationEventPropertiesEditionPart) provider.getPropertiesEditionPart(
							UMLViewsRepository.ReceiveOperationEvent.class, kind, this);
	...
	public void setPropertiesEditionPart(java.lang.Class key, int kind, IPropertiesEditionPart propertiesEditionPart) {
		if (key == UMLViewsRepository.ReceiveOperationEvent.class)
			this.basePart = (ReceiveOperationEventPropertiesEditionPart) propertiesEditionPart;
	}
	...
	public void initPart(java.lang.Class key, int kind, EObject elt, ResourceSet allResource) {
		if (basePart != null && key == UMLViewsRepository.ReceiveOperationEvent.class) {
			...
			basePart.initVisibility((EEnum) UMLPackage.eINSTANCE.getNamedElement_Visibility().getEType(),
					receiveOperationEvent.getVisibility());
			basePart.initClientDependency(receiveOperationEvent, null, UMLPackage.eINSTANCE
	...
	public CompoundCommand getPropertiesEditionCommand(EditingDomain editingDomain) {
		...
		if (receiveOperationEvent != null) {
			...
			for (Iterator iter = ownedCommentToAddFromOwnedComment.iterator(); iter.hasNext();)
				cc.append(AddCommand.create(editingDomain, receiveOperationEvent, UMLPackage.eINSTANCE
			...
			for (Iterator iter = ownedCommentToMoveFromOwnedComment.iterator(); iter.hasNext();) {
				org.eclipse.emf.eef.runtime.impl.utils.EMFListEditUtil.MoveElement moveElement = (org.eclipse.emf.eef.runtime.impl.utils.EMFListEditUtil.MoveElement) iter
						.next();
				cc.append(MoveCommand.create(editingDomain, receiveOperationEvent, UMLPackage.eINSTANCE.getComment(),
						moveElement.getElement(), moveElement.getIndex()));
			}
			cc.append(SetCommand.create(editingDomain, receiveOperationEvent, UMLPackage.eINSTANCE
			...
			for (Iterator iter = clientDependencyToRemoveFromClientDependency.iterator(); iter.hasNext();)
				cc.append(RemoveCommand.create(editingDomain, receiveOperationEvent, UMLPackage.eINSTANCE
						.getNamedElement_ClientDependency(), iter.next()));
			...
			// cc.append(MoveCommand.create(editingDomain, receiveOperationEvent,
		...
		if (PropertiesEditionEvent.COMMIT == event.getState()
			...
			if (UMLViewsRepository.ReceiveOperationEvent.ownedComment == event.getAffectedEditor()) {
				if (PropertiesEditionEvent.SET == event.getKind()) {
					...
					command.append(AddCommand.create(liveEditingDomain, receiveOperationEvent, UMLPackage.eINSTANCE
							.getElement_OwnedComment(), event.getNewValue()));
				else if (PropertiesEditionEvent.REMOVE == event.getKind())
					command.append(DeleteCommand.create(liveEditingDomain, event.getNewValue()));
				else if (PropertiesEditionEvent.MOVE == event.getKind())
					command.append(MoveCommand.create(liveEditingDomain, receiveOperationEvent, UMLPackage.eINSTANCE
							.getComment(), event.getNewValue(), event.getNewIndex()));
			}
			if (UMLViewsRepository.ReceiveOperationEvent.name == event.getAffectedEditor())
				command.append(SetCommand.create(liveEditingDomain, receiveOperationEvent, UMLPackage.eINSTANCE
			...
			if (UMLViewsRepository.ReceiveOperationEvent.clientDependency == event.getAffectedEditor()) {
				...
				if (PropertiesEditionEvent.REMOVE == event.getKind())
					command.append(RemoveCommand.create(liveEditingDomain, receiveOperationEvent, UMLPackage.eINSTANCE
							.getNamedElement_ClientDependency(), event.getNewValue()));
