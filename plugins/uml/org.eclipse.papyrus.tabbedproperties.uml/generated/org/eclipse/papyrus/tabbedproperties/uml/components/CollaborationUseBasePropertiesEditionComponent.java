 ...
 */
public class CollaborationUseBasePropertiesEditionComponent extends StandardPropertiesEditionComponent {
	 ...
	 * The Base part
	 */
	private CollaborationUsePropertiesEditionPart basePart;
	...
	private AdapterImpl initializeSemanticAdapter() {
		return new EContentAdapter() {
			...
			public void notifyChanged(Notification msg) {
				...
				if (msg.getFeature() != null
						&& (((EStructuralFeature) msg.getFeature()) == UMLPackage.eINSTANCE.getCollaborationUse_RoleBinding() || ((EStructuralFeature) msg.getFeature()).getEContainingClass() == UMLPackage.eINSTANCE
								.getDependency())) {
	...
	public java.lang.Class translatePart(String key) {
		if (BASE_PART.equals(key))
			return UMLViewsRepository.CollaborationUse.class;
	...
	public CompoundCommand getPropertiesEditionCommand(EditingDomain editingDomain) {
		...
		if (collaborationUse != null) {
			...
			// }
			List roleBindingToAdd = basePart.getRoleBindingToAdd();
			for (Iterator iter = roleBindingToAdd.iterator(); iter.hasNext();)
			...
			Map roleBindingToRefresh = basePart.getRoleBindingToEdit();
			for (Iterator iter = roleBindingToRefresh.keySet().iterator(); iter.hasNext();) {

				// Start of user code for roleBinding reference refreshment

				Dependency nextElement = (Dependency) iter.next();
				Dependency roleBinding = (Dependency) roleBindingToRefresh.get(nextElement);

				// End of user code
			}
			List roleBindingToRemove = basePart.getRoleBindingToRemove();
			for (Iterator iter = roleBindingToRemove.iterator(); iter.hasNext();)
				cc.append(DeleteCommand.create(editingDomain, iter.next()));
			List roleBindingToMove = basePart.getRoleBindingToMove();
			for (Iterator iter = roleBindingToMove.iterator(); iter.hasNext();) {
				org.eclipse.emf.eef.runtime.impl.utils.EMFListEditUtil.MoveElement moveElement = (org.eclipse.emf.eef.runtime.impl.utils.EMFListEditUtil.MoveElement) iter.next();
				cc.append(MoveCommand.create(editingDomain, collaborationUse, UMLPackage.eINSTANCE.getDependency(), moveElement.getElement(), moveElement.getIndex()));
			}
	...
	public String getHelpContent(String key, int kind) {
		...
		if (key == UMLViewsRepository.CollaborationUse.roleBinding)
			return "A mapping between features of the collaboration type and features of the classifier or operation. This mapping indicates which connectable element of the classifier or operation plays which role(s) in the collaboration. A connectable element may be bound to multiple roles in the same collaboration use (that is, it may play multiple roles)."; //$NON-NLS-1$
	...
	public void dispose() {
		if (semanticAdapter != null)
			collaborationUse.eAdapters().remove(semanticAdapter);
	}
