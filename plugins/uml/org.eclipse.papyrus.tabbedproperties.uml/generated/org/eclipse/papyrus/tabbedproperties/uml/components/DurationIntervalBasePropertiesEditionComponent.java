 ...
 */
public class DurationIntervalBasePropertiesEditionComponent extends StandardPropertiesEditionComponent {
	 ...
	 * The Base part
	 */
	private DurationIntervalPropertiesEditionPart basePart;
	...
	public IPropertiesEditionPart getPropertiesEditionPart(int kind, String key) {
		if (durationInterval != null && BASE_PART.equals(key)) {
			if (basePart == null) {
				...
				if (provider != null) {
					basePart = (DurationIntervalPropertiesEditionPart) provider.getPropertiesEditionPart(
							UMLViewsRepository.DurationInterval.class, kind, this);
	...
	public void initPart(java.lang.Class key, int kind, EObject elt, ResourceSet allResource) {
		if (basePart != null && key == UMLViewsRepository.DurationInterval.class) {
			...
			DurationInterval durationInterval = (DurationInterval) elt;
			// init values
			...
			basePart.initVisibility((EEnum) UMLPackage.eINSTANCE.getNamedElement_Visibility().getEType(),
					durationInterval.getVisibility());
			basePart.initClientDependency(durationInterval, null, UMLPackage.eINSTANCE
	...
	public CompoundCommand getPropertiesEditionCommand(EditingDomain editingDomain) {
		...
		if (durationInterval != null) {
			...
			for (Iterator iter = ownedCommentToAddFromOwnedComment.iterator(); iter.hasNext();)
				cc.append(AddCommand.create(editingDomain, durationInterval, UMLPackage.eINSTANCE
			...
			for (Iterator iter = ownedCommentToMoveFromOwnedComment.iterator(); iter.hasNext();) {
				org.eclipse.emf.eef.runtime.impl.utils.EMFListEditUtil.MoveElement moveElement = (org.eclipse.emf.eef.runtime.impl.utils.EMFListEditUtil.MoveElement) iter
						.next();
				cc.append(MoveCommand.create(editingDomain, durationInterval, UMLPackage.eINSTANCE.getComment(),
						moveElement.getElement(), moveElement.getIndex()));
			}
			cc.append(SetCommand.create(editingDomain, durationInterval, UMLPackage.eINSTANCE.getNamedElement_Name(),
					basePart.getName()));

			cc.append(SetCommand.create(editingDomain, durationInterval, UMLPackage.eINSTANCE
			...
			for (Iterator iter = clientDependencyToRemoveFromClientDependency.iterator(); iter.hasNext();)
				cc.append(RemoveCommand.create(editingDomain, durationInterval, UMLPackage.eINSTANCE
						.getNamedElement_ClientDependency(), iter.next()));
			...
			// cc.append(MoveCommand.create(editingDomain, durationInterval,
	...
	public EObject getPropertiesEditionObject(EObject source) {
		if (source instanceof DurationInterval) {
			...
			durationIntervalToUpdate.setVisibility((VisibilityKind) basePart.getVisibility());

			durationIntervalToUpdate.getClientDependencies().addAll(basePart.getClientDependencyToAdd());
		...
		if (PropertiesEditionEvent.COMMIT == event.getState()
			...
			if (UMLViewsRepository.DurationInterval.ownedComment == event.getAffectedEditor()) {
				if (PropertiesEditionEvent.SET == event.getKind()) {
					...
					command.append(AddCommand.create(liveEditingDomain, durationInterval, UMLPackage.eINSTANCE
							.getElement_OwnedComment(), event.getNewValue()));
				else if (PropertiesEditionEvent.REMOVE == event.getKind())
					command.append(DeleteCommand.create(liveEditingDomain, event.getNewValue()));
				else if (PropertiesEditionEvent.MOVE == event.getKind())
					command.append(MoveCommand.create(liveEditingDomain, durationInterval, UMLPackage.eINSTANCE
							.getComment(), event.getNewValue(), event.getNewIndex()));
			}
			if (UMLViewsRepository.DurationInterval.name == event.getAffectedEditor())
				command.append(SetCommand.create(liveEditingDomain, durationInterval, UMLPackage.eINSTANCE
			...
			if (UMLViewsRepository.DurationInterval.clientDependency == event.getAffectedEditor()) {
				...
				if (PropertiesEditionEvent.REMOVE == event.getKind())
					command.append(RemoveCommand.create(liveEditingDomain, durationInterval, UMLPackage.eINSTANCE
							.getNamedElement_ClientDependency(), event.getNewValue()));
	...
	public String getHelpContent(String key, int kind) {
		if (key == UMLViewsRepository.DurationInterval.ownedComment)
			return null; //$NON-NLS-1$
		if (key == UMLViewsRepository.DurationInterval.name)
			return null; //$NON-NLS-1$
		if (key == UMLViewsRepository.DurationInterval.visibility)
			return null; //$NON-NLS-1$
		if (key == UMLViewsRepository.DurationInterval.clientDependency)
			return null; //$NON-NLS-1$
	...
	public Diagnostic validate() {
		if (IPropertiesEditionComponent.BATCH_MODE.equals(editing_mode)) {
			...
			return Diagnostician.INSTANCE.validate(durationInterval);
		else
			return null;
	}
