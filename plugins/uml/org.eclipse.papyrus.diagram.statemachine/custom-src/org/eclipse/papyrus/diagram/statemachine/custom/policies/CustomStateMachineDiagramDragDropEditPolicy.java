...
public class CustomStateMachineDiagramDragDropEditPolicy extends CommonDiagramDragDropEditPolicy {
	...
	/**
	 * Instantiates a new state machine diagram drag drop edit policy with the right link mapping
	 * helper
	 */
	 ...
	 */
	protected Command dropStateMachine(DropObjectsRequest dropRequest, Point location, StateMachine droppedElement,
			int nodeVISUALID) {
		...
		if ((graphicalParentObject instanceof org.eclipse.uml2.uml.Package)
				&& (((org.eclipse.uml2.uml.Package) graphicalParentObject).getPackagedElements()
			...
			CreateCommand createStateMachine = new CreateCommand(getEditingDomain(), descriptor, (View) (getHost()
					.getModel()));
			...
			CustomStateMachineWithDefaultRegionCreateNodeCommand createRegion = new CustomStateMachineWithDefaultRegionCreateNodeCommand(
					...
					((IGraphicalEditPart) getHost()).getDiagramPreferencesHint(), getEditingDomain(),
					DiagramUIMessages.CreateCommand_Label, createStateMachine.getAffectedFiles());

			CustomStateMachineSetBoundsCommand setBoundsCommand = new CustomStateMachineSetBoundsCommand(
					getEditingDomain(), null, descriptor, new Rectangle(location.x, location.y, -1, -1));
	...
	protected Command dropRegion(DropObjectsRequest dropRequest, Region droppedElement, int nodeVISUALID) {
		...
		if ((graphicalParentObject instanceof Region)
				&& (((Region) graphicalParentObject).getStateMachine().getRegions().contains(droppedElement))
			...
			// check whether the dropped region is already shown in the state machine
			...
			if (alreadyShown != null) {
				...
				CustomRegionDeleteCommand deleteAlreadyShown = new CustomRegionDeleteCommand(getEditingDomain(),
						alreadyShown);
			...
			CustomRegionCreateElementCommand createNewRegion = new CustomRegionCreateElementCommand(
					adaptableForExistingRegionView, adaptableForDroppedRegion, ((IGraphicalEditPart) getHost())
							.getDiagramPreferencesHint(), getEditingDomain(), DiagramUIMessages.CreateCommand_Label,
					dropLocation);
			...
			for (Iterator iter = changeBoundsRequest.getEditParts().iterator(); iter.hasNext();) {
				...
				if (element instanceof RegionEditPart) {
					...
					if (targetEP != null) {
						...
						dropLocation = Zone.getZoneFromLocationInRectangleWithAbsoluteCoordinates(mouseLocation,
								targetFigBounds);
		...
		if (request instanceof DropObjectsRequest) {
			...
			for (Iterator iter = dropRequest.getObjects().iterator(); iter.hasNext();) {
				...
				if (element instanceof Region) {
					// check whether the dropped region is already shown in the state machine
					View stateMachineCompartment = (View) ((StateMachineCompartmentEditPart) getHost().getParent()
							.getParent()).getModel();
					View alreadyShown = null;
					Iterator<View> it = stateMachineCompartment.getChildren().iterator();
					while ((alreadyShown == null) && it.hasNext()) {
