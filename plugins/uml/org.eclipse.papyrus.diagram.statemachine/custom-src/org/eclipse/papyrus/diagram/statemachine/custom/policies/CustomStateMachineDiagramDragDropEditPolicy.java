...
public class CustomStateMachineDiagramDragDropEditPolicy extends
	 ...
	 */
	protected Command dropRegion(DropObjectsRequest dropRequest,
			Region droppedElement, int nodeVISUALID) {
		...
		if ((graphicalParentObject instanceof Region)
				&& (((Region) graphicalParentObject).getStateMachine()
						.getRegions().contains(droppedElement))
				&& !graphicalParentObject.equals(droppedElement)) {
			CompositeCommand cc = new CompositeCommand("Drop");
			// get an adaptable for the dropped region
			IAdaptable adaptableForDroppedRegion = (IAdaptable) new SemanticAdapter(
					droppedElement, null);
			// get the existing region view
			View existingRegionView = (View) graphicalParentEditPart
					.getParent().getModel();
			// get and adaptable for it, to pass on to commands
			IAdaptable adaptableForExistingRegionView = (IAdaptable) new SemanticAdapter(
					null, existingRegionView);

			// check whether the dropped region is already shown in the state
			// machine
			View stateMachineCompartment = (View) existingRegionView
					.eContainer();
			View alreadyShown = null;
			Iterator<View> it = stateMachineCompartment.getChildren()
					.iterator();
			...
			if (alreadyShown != null) {
				...
				CustomRegionDeleteCommand deleteAlreadyShown = new CustomRegionDeleteCommand(
						getEditingDomain(), alreadyShown);
				cc.compose(deleteAlreadyShown);
			}

			// do the whole job
			CustomRegionCreateElementCommand createNewRegion = new CustomRegionCreateElementCommand(
					adaptableForExistingRegionView, adaptableForDroppedRegion,
					((IGraphicalEditPart) getHost())
							.getDiagramPreferencesHint(), getEditingDomain(),
	 ...
	 * @return the drop command
	 */
	protected Command dropStateMachine(DropObjectsRequest dropRequest,
			Point location, StateMachine droppedElement, int nodeVISUALID) {
		...
		if ((graphicalParentObject instanceof org.eclipse.uml2.uml.Package)
				&& (((org.eclipse.uml2.uml.Package) graphicalParentObject)
						.getPackagedElements().contains(droppedElement))) {
			CompositeCommand cc = new CompositeCommand("Drop");
			...
			CreateCommand createStateMachine = new CreateCommand(
			...
			CustomStateMachineSetBoundsCommand setBoundsCommand = new CustomStateMachineSetBoundsCommand(
					getEditingDomain(), null, descriptor, new Rectangle(
	...
	public int getNodeVisualID(View containerView, EObject domainElement) {
		if ((domainElement instanceof Region)
				&& (containerView.getElement() instanceof Region)) {
	...
	protected Command getSpecificDropCommand(DropObjectsRequest dropRequest,
		...
		case StateMachineEditPart.VISUAL_ID:
			return dropStateMachine(dropRequest, location,
					(StateMachine) semanticElement, nodeVISUALID);
		case RegionEditPart.VISUAL_ID:
			return dropRegion(dropRequest, (Region) semanticElement,
					nodeVISUALID);

		default:
			return super.getSpecificDropCommand(dropRequest, semanticElement,
	...
	public void showTargetFeedback(Request request) {
		if ((request instanceof ChangeBoundsRequest)
			...
			for (Iterator iter = changeBoundsRequest.getEditParts().iterator(); iter
					.hasNext();) {
				...
				if (element instanceof RegionEditPart) {
					...
					View stateMachineCompartment = (View) ((View) element
							.getModel()).eContainer();
					if (stateMachineCompartment.getChildren().size() == 1)
						return;

					CustomRegionDragTracker dragTracker = regionEditPart
							.getRegionDragTracker();
					RegionEditPart targetEP = dragTracker
							.getTargetRegionEditPart();
					if (targetEP != null) {
						RegionFigure targetFig = targetEP.getPrimaryShape();

						// make a local copy
						Rectangle targetFigBounds = targetFig.getBounds()
								.getCopy();
						...
						dropLocation = Zone
								.getZoneFromLocationInRectangleWithAbsoluteCoordinates(
										mouseLocation, targetFigBounds);
						...
						if (Zone.isTop(dropLocation)) {
							targetFigBounds.setSize(targetFigBounds.getSize()
									.scale(1.0, 0.5));
							...
							targetFigBounds.setSize(targetFigBounds.getSize()
									.scale(1.0, 0.5));
							targetFigBounds
									.translate(0, targetFigBounds.height);
						}

						getSizeOnDropFeedback().setBounds(
								new PrecisionRectangle(targetFigBounds));
					} else {

						Rectangle targetFigBounds = dragTracker
								.getRegionFigureBounds();
		...
		if (request instanceof DropObjectsRequest) {
			...
			for (Iterator iter = dropRequest.getObjects().iterator(); iter
					.hasNext();) {
				EObject element = (EObject) iter.next();
				if (element instanceof Region) {
					// check whether the dropped region is already shown in the
					// state machine
					View stateMachineCompartment = (View) ((StateMachineCompartmentEditPart) getHost()
					...
					if (alreadyShown == null) {
						RegionFigure targetFig = ((RegionEditPart) getHost()
								.getParent()).getPrimaryShape();
						...
						targetFig.translateToAbsolute(targetFigBounds);
						// retrieve mouse location
						Point mouseLocation = dropRequest.getLocation()
								.getCopy();
