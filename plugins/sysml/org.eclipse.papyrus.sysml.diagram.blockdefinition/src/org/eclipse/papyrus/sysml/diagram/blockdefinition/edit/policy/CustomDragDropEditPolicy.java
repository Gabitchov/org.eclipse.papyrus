...
public class CustomDragDropEditPolicy extends ClassDiagramDragDropEditPolicy {
	...
	public int getNodeVisualID(View containerView, EObject domainElement) {
		...
		if(containerView instanceof Diagram) { // Top Nodes
			if(domainElement instanceof org.eclipse.uml2.uml.Class && !((org.eclipse.uml2.uml.Class)domainElement).getApplicableStereotypes().isEmpty()) {
				semanticHint = BlockDefinitionDiagramElementTypes.BLOCK.getSemanticHint();
			}

		} else { // Child Nodes
			if(domainElement instanceof Port) {
				semanticHint = BlockDefinitionDiagramElementTypes.PORT_CN.getSemanticHint();
			} else if(domainElement instanceof Property) {
//				semanticHint = BlockDefinitionDiagramElementTypes.PROPERTY_CN.getSemanticHint();
			} 
		}
