...
public class StereotypeComposite extends DecoratedTreeComposite implements ISelectionChangedListener {
	...
	public void setInput(RootElementTreeObject element) {
		...
		if(Activator.getDefault().getPreferenceStore().getBoolean(ProfilePreferenceConstants.EXPAND_STERETOYPES_TREE)) {
	...
	public void addButtonPressed() {
		...
		if(result == ChooseSetStereotypeDialog.OK) {
			...
			EList newOrderList = new BasicEList();
			...
			if(!(newStereotypeList.equals(oldStereotypeList))) {
				...
				Iterator it = oldStereotypeList.iterator();
				while(it.hasNext()) {
				...
				// apply others
				Iterator newApplyStereotypes = newStereotypeList.iterator();
