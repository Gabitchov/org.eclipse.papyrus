...
import com.cea.papyrus.profile.tree.objects.ValueTreeObject;
...
import com.cea.papyrus.profile.ui.listeners.DoubleClickListener;
...
public class PropertyComposite extends DecoratedTreeComposite {
	 ...
	 * The parent panel.
	 */
	protected StereotypePanel parentPanel;
	
	/**
	 * Creates a new PropertyComposite.
	 * @param parent the parent StereotypePanel
	 */
	public PropertyComposite(StereotypePanel parent) {
	...
	protected StereotypePanel getParentPanel() {
		return parentPanel;
	}
	...
	public void refresh() {
		...
		if(treeViewer.getTree()!= null && !(treeViewer.getTree().isDisposed())) {
			...
			PropertyTreeObject pTO = (PropertyTreeObject) treeViewer.getInput();
			if (pTO == null) {
	...
	/**
	 * Action triggered when the add button
	 * is pressed.
	 */
	@Override
	public void addButtonPressed() {
		...
		Stereotype selectedSt = ((StereotypeTreeObject) pTO.getParent()).getStereotype();
		Element selectedElt   = ((RootElementTreeObject) pTO.getParent().getParent()).getElement();
		...
		if (lower == upper && selectedElt.getValue(selectedSt, selectedProp.getName())!=null) {
			if(selectedElt.getValue(selectedSt, selectedProp.getName()) instanceof EList) {
				if(((EList)selectedElt.getValue(selectedSt, selectedProp.getName())).size()>=upper) {
					Message.warning(
							"Multiplicity of this property is "
							+ selectedProp.getLower()+".."
							+ selectedProp.getUpper()+"\n"
...
//				newValue = profile.create((Classifier) uml2Type);
	...
	/**
	 * Action triggered when the remove button
	 * is pressed.
	 */
	@Override
	...
	/**
	 * Action triggered when the up button
	 * is pressed.
	 */
	@Override
	...
	/**
	 * Action triggered when the down button
	 * is pressed.
	 */
	@Override
	 ...
	 */
	public void setInput(PropertyTreeObject element) {
