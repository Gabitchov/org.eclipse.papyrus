 ...
 */
public class NamedElementFilter extends ViewerFilter {
	...
	public boolean select(Viewer viewer, Object parentElement, Object element) {
		...
		if(element instanceof Resource){
			...
			while(iter.hasNext()){
				if (iter.next() instanceof org.eclipse.uml2.uml.NamedElement){
					return true;
				}
			}
		}
		...
		if(element instanceof NamedElement){
			Iterator<Element> iter=(((NamedElement)element)).allOwnedElements().iterator();
			while(iter.hasNext()){
				if (iter.next() instanceof org.eclipse.uml2.uml.NamedElement){
					return true;
				}
