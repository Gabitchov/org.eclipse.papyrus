 ...
 * 
 */
public class NotifyingWorkspaceCommandStack extends
		AbstractTransactionalCommandStack// AbstractTransactionalCommandStack
	...
	/**
	 * HAS CHANGE FROM ORIGINAL IMPLEMENTATION TO USE
	 * {@link EditingDomainUndoContext}
	 */
	...
	public void addCommandStackListener(final CommandStackListener listener) {
		...
		IOperationHistoryListener proxy = new IOperationHistoryListener() {
			...
			public void historyNotification(OperationHistoryEvent event) {
				...
				// finished
				if (OperationHistoryEvent.DONE == type
						|| OperationHistoryEvent.REDONE == type
						|| OperationHistoryEvent.UNDONE == type) {
	...
	public void removeCommandStackListener(CommandStackListener listener) {
		IOperationHistoryListener proxy = proxyOperationListeners
				.remove(listener);
		if (proxy != null) {
	 ...
	 * editing domain. HAS CHANGE FROM ORIGINAL IMPLEMENTATION TO USE
	 * {@link EditingDomainUndoContext}
	 */
	@Override
	public void setEditingDomain(InternalTransactionalEditingDomain domain) {
		 ...
		 */
		if (getDomain() != null) {
			boolean domainHasChanged = oldDomain == null
					|| !oldDomain.equals(getDomain());
			if (domainHasChanged){
				defaultContext = new EditingDomainUndoContext(domain,
	...
	protected void doExecute(Command command, Map<?, ?> options)
		...
		EMFCommandOperation oper = new EMFCommandOperation(getDomain(),
				command, options);
		...
		try {
			IStatus status = history.execute(oper, new NullProgressMonitor(),
					null);
	...
	public void undo() {
		try {
			getOperationHistory().undo(getDefaultUndoContext(),
	...
	public void redo() {
		try {
			getOperationHistory().redo(getDefaultUndoContext(),
					new NullProgressMonitor(), null);
		} catch (ExecutionException e) {
	...
	public void flush() {
		getOperationHistory()
				.dispose(getDefaultUndoContext(), true, true, true);
	...
	public Command getUndoCommand() {
		...
		IUndoableOperation topOperation = getOperationHistory()
				.getUndoOperation(getDefaultUndoContext());
	...
	public Command getRedoCommand() {
		...
		IUndoableOperation topOperation = getOperationHistory()
				.getRedoOperation(getDefaultUndoContext());
	...
	public void executeTriggers(Command command, List<Command> triggers,
		...
		if (!triggers.isEmpty()) {
			TriggerCommand trigger = (command == null) ? new TriggerCommand(
					triggers) : new TriggerCommand(command, triggers);
			...
			try {
				...
				InternalTransaction parent = (InternalTransaction) tx
						.getParent();
				...
				Tracing.catching(WorkspaceCommandStackImpl.class,
						"executeTriggers", e); //$NON-NLS-1$
	 ...
	 * resources are changed by an operation and attaches the appropriate
	 * {@link ResourceUndoContext} to it when it completes.
	 * 
	 * @author Christian W. Damus (cdamus)
	 */
	private class DomainListener extends ResourceSetListenerImpl implements
		...
		public void resourceSetChanged(ResourceSetChangeEvent event) {
			...
			Set<Resource> unloaded = getUnloadedResources(event
					.getNotifications());
			if (unloaded != null) {
				// dispose their undo contexts
				for (Resource next : unloaded) {
					getOperationHistory().dispose(
							new ResourceUndoContext(getDomain(), next), true,
							true, true);
				}
			}
		 ...
		 */
		private Set<Resource> getUnloadedResources(
	...
	public boolean isSaveNeeded() {
		...
		IUndoableOperation nextUndoableOperation = history
				.getUndoOperation(getDefaultUndoContext());
		...
		return savedContext != null ? !nextUndoableOperation
				.hasContext(getSavedContext()) : true;
	}
	...
	public void saveIsDone() {
		...
		if (savedContext != null) {
			...
			IUndoableOperation[] undoableOperations = history
					.getUndoHistory(getSavedContext());
			...
			IUndoableOperation[] redoableOperations = history
					.getRedoHistory(getSavedContext());
