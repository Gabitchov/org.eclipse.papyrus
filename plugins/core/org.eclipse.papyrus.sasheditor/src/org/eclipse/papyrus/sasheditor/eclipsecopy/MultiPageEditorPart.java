...
/**
 * Copy used as base class of the main editor. A multi-page editor is an editor with multiple pages, each of which may contain an editor or an
 * arbitrary SWT control.
 * <p>
...
public abstract class MultiPageEditorPart extends EditorPart implements IMultiPageEditorPart {
	...
	/**
	 * List of nested editors. Element type: IEditorPart. Need to hang onto them here, in addition to using get/setData on the items, because
	 * dispose() needs to access them, but widgetry has already
	...
	/**
	 * Creates and adds a new page containing the given control to this multi-page editor. The control may be <code>null</code>, allowing it to be
	 * created and set later using <code>setControl</code>.
	 * 
	 * @param control
	...
	// }
	/**
	 * Creates and adds a new page containing the given control to this multi-page editor. The page is added at the given index. The control may be
	 * <code>null</code>, allowing it to be created and set
	...
	// }
	/**
	 * Creates and adds a new page containing the given editor to this multi-page editor. This also hooks a property change listener on the nested
	 * editor.
	 * 
	 * @param editor
	 *        the nested editor
	...
	// }
	/**
	 * Creates and adds a new page containing the given editor to this multi-page editor. The page is added at the given index. This also hooks a
	 * property change listener on the nested editor.
	 * 
	 * @param index
	 *        the index at which to add the page (0-based)
	 * @param editor
	...
	// }
	/**
	 * Initialize the MultiPageEditorPart to use the page switching command. Clients can override this method with an empty body if they wish to
	 * opt-out.
	 * 
	 * @since 3.4
	 */
	...
	// }
	/**
	 * Creates the site for the given nested editor. The <code>MultiPageEditorPart</code> implementation of this method creates an instance of
	 * <code>MultiPageEditorSite</code>. Subclasses may
	...
	// }
	/**
	 * Returns the text label for the page with the given index. Returns the empty string if no text label has been set for the page. The page index
	 * must be valid.
	 * 
	 * @param pageIndex
	 *        the index of the page
	...
	// }
	/**
	 * Handles a property change notification from a nested editor. The default implementation simply forwards the change to listeners on this
	 * multi-page editor by calling <code>firePropertyChange</code> with the same property id. For example, if the dirty state of a nested editor
	 * changes (property id <code>IEditorPart.PROP_DIRTY</code>), this method handles it
	...
	/**
	 * The <code>MultiPageEditorPart</code> implementation of this <code>IEditorPart</code> method sets its site to the given site, its input to the
	 * given input, and the site's selection provider to a <code>MultiPageSelectionProvider</code>. Subclasses may extend this method.
	 * 
	 * @param site
	...
	// }
	/**
	 * This method can be used by implementors of {@link MultiPageEditorPart#createPageContainer(Composite)} to deactivate the active inner editor
	...
	// }
	/**
	 * Removes the page with the given index from this multi-page editor. The controls for the page are disposed of; if the page has an editor, it is
	...
	// }
	/**
	 * The <code>MultiPageEditor</code> implementation of this <code>IWorkbenchPart</code> method sets focus on the active nested editor, if there is
	 * one.
	 * <p>
	 ...
	 */
	abstract public IEditorPart getActiveEditor();
}
