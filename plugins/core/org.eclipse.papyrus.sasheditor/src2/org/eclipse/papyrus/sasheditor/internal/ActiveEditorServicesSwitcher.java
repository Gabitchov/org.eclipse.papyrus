...
/**
 * This class is used to switch the current Active Editor. When a new Editor is set active, by
 * calling {@link #setActiveEditor(PagePart)}, following actions are performed:
 * <ul>
 * <li>deactivate services: nested site and keybinding</li>
 * <li>An event is sent to the ActionBarContributor if it accept it (by implementing
 * {@link IMultiPageEditorActionBarContributor})</li>
 * <li>Send {@link SelectionChangedEvent} to the main editor. The event contains the current
 * selection of the new active editor.</li>
...
public class ActiveEditorServicesSwitcher implements IActiveEditorChangedListener {
	...
	/**
	 * Change the current selection of the outermost editor (the main editor). Send a
	 * {@link SelectionChangedEvent} event to the outerProvider. The event contains the current
	 * selection of the new activeEditor.
	 * 
	 * @param editor
	...
	private void propagateSelectionChanged() {
		...
		// Get the selection of the new activeEditor and send an SelectionChangedEvent to the
		// outerProvider (provider of the main
