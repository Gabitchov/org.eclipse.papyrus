...
public class ObjectFlowEditPart extends ConnectionNodeEditPart implements ITreeBranchEditPart {
	...
	protected void createDefaultEditPolicies() {
		...
		installEditPolicy(EditPolicy.CONNECTION_ROLE, new ConnectionEditPolicy() {
			...
			protected Command createDeleteViewCommand(GroupRequest deleteRequest) {
				...
				command = command.chain(new ICommandProxy(new RemoveEObjectReferencesFromDiagram(getEditingDomain(), ObjectFlowEditPart.this.getDiagramView(), Collections
	...
	protected boolean addFixedChild(EditPart childEditPart) {
		if (childEditPart instanceof ObjectFlowNameEditPart) {
			((ObjectFlowNameEditPart) childEditPart).setLabel(getPrimaryShape().getFigureActivityEdgeFigure_name());
			return true;
		}
		return false;
	}
