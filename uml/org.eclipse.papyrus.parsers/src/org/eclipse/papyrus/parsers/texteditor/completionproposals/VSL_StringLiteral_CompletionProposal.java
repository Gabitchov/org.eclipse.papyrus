...
public class VSL_StringLiteral_CompletionProposal implements ICompletionProposalComputer {
	...
	final static private String[] TVL_StringLiteral_StringsInfo = {
			NFPMessages.StringLiteral_StringsInfo,
	}; 

	/** Text displayed in the completion area window */
	final static private String[] TVL_StringLiteral_StringName = {
		NFPMessages.StringLiteral_StringName,
	};
		...
		for(int i=0; i < TVL_StringLiteral_Strings.length; i++) {
			if(TVL_StringLiteral_Strings[i].startsWith(prefix)) {
				proposal = new CompletionProposal(
						TVL_StringLiteral_Strings[i],
						documentOffset-prefix.length(),
						prefix.length()+selectionRange,
						TVL_StringLiteral_Strings[i].length()-1,
						null,
						TVL_StringLiteral_StringName[i],
						null,
						TVL_StringLiteral_StringsInfo[i]);
				v.add(proposal);
			}
		}
