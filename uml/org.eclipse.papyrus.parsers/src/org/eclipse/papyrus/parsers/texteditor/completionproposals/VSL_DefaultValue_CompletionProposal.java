...
public class VSL_DefaultValue_CompletionProposal implements ICompletionProposalComputer {
	...
	final static private String[] TVL_DefaultValue_StringsInfo = {
			NFPMessages.DefaultValue_StringsInfo,
	}; 

	/** Text displayed in the completion area window */
	final static private String[] TVL_DefaultValue_StringName = {
			NFPMessages.DefaultValue_StringName,
	};
		...
		for(int i=0; i < TVL_DefaultValue_Strings.length; i++) {
			if(TVL_DefaultValue_Strings[i].startsWith(prefix)) {
				proposal = new CompletionProposal(
						TVL_DefaultValue_Strings[i],
						documentOffset-prefix.length(),
						prefix.length()+selectionRange,
						TVL_DefaultValue_Strings[i].length(),
						null,
						TVL_DefaultValue_StringName[i],
						null,
						TVL_DefaultValue_StringsInfo[i]);
				v.add(proposal);
			}
		}
