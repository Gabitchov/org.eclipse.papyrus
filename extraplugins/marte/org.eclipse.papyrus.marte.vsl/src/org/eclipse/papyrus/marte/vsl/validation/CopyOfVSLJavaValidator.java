...
public class CopyOfVSLJavaValidator extends AbstractVSLJavaValidator {
...
//	}
	...
	//TODO: Fix the value of these fields when the editor is launched from the MARTE plug-in
	private static Namespace model ;
	private static Element contextElement ;
	private static Type expectedType ;
	//private static ExpectedKind expectedKind ;
	...
	//private static HashMap<Element, Type> expectedTypesBasedOnContext ;
	...
	public static void init(Element _contextElement) {
		...
		if (contextElement != null) {
			...
			for (Package importedPackage : model.getImportedPackages()) {
				//System.out.println("imported " + importedPackage.getName()) ;
				if (!typesResolved)
					typesResolved = initPredefinedTypes(importedPackage) ;
			}
			if (typesResolved) initPredefinedOpSignatures() ;
		}
		//expectedTypesBasedOnContext = new HashMap<Element, Type>() ;
	}
	...
	public static void setExpectedType (Type _expectedType) {
		...
		for (Stereotype s : expectedType.getAppliedStereotypes()) {
...
//	public static void setExpectedTypeBasedOnContext (Element context, Type expectedType) {
//		expectedTypesBasedOnContext.put(context, expectedType) ;
//	}
	
//	public static Type getExpectedTypeBasedOnContext(EObject rule) {
//		return expectedTypesBasedOnContext.get(rule) ;
//	}
...
//			return ;
	...
	public void ConditionExpression (ConditionalExpression exp) {
		if (exp.getOp() != null && !exp.getOp().isEmpty() && exp.getOp().get(0) == "?") {
