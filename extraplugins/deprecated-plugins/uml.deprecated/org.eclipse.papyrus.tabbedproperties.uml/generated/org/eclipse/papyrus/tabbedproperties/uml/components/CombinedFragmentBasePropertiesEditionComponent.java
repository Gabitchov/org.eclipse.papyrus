...
public class CombinedFragmentBasePropertiesEditionComponent extends StandardPropertiesEditionComponent {
			...
			public void notifyChanged(Notification msg) {
				if(basePart == null)
					...
					if(msg.getFeature() != null &&
							...
							|| ((EStructuralFeature)msg.getFeature()).getEContainingClass() == UMLPackage.eINSTANCE.getCombinedFragment_Operand())) {
					...
					if(msg.getFeature() != null &&
							...
							|| ((EStructuralFeature)msg.getFeature()).getEContainingClass() == UMLPackage.eINSTANCE.getCombinedFragment_CfragmentGate())) {
	...
	public CompoundCommand getPropertiesEditionCommand(EditingDomain editingDomain) {
		...
		if(combinedFragment != null) {
			...
			for(Iterator iter = cfragmentGateToRefreshFromCfragmentGate.keySet().iterator(); iter.hasNext();) {
				...
				for(EStructuralFeature feature : nextElement.eClass().getEAllStructuralFeatures()) {
					if(feature.isChangeable() && !(feature instanceof EReference && ((EReference)feature).isContainer())) {
						cc.append(SetCommand.create(editingDomain, nextElement, feature, cfragmentGate.eGet(feature)));
					}
				}
	...
	public Diagnostic validate() {
		...
		if(IPropertiesEditionComponent.BATCH_MODE.equals(editing_mode)) {
			...
			validate = Diagnostician.INSTANCE.validate(combinedFragment);
