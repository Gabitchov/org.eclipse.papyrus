...
public class ProtocolStateMachineBasePropertiesEditionComponent extends StandardPropertiesEditionComponent {
		...
		return new EContentAdapter() {
			...
			public void notifyChanged(Notification msg) {
				if(basePart == null)
					...
					if(msg.getFeature() != null &&
							(((EStructuralFeature)msg.getFeature()) == UMLPackage.eINSTANCE.getProtocolStateMachine_Conformance()
							|| ((EStructuralFeature)msg.getFeature()).getEContainingClass() == UMLPackage.eINSTANCE.getProtocolStateMachine_Conformance())) {
	...
	public CompoundCommand getPropertiesEditionCommand(EditingDomain editingDomain) {
		...
		if(protocolStateMachine != null) {
			...
			for(Iterator iter = conformanceToRefreshFromConformance.keySet().iterator(); iter.hasNext();) {
				...
				ProtocolConformance conformance = (ProtocolConformance)conformanceToRefreshFromConformance.get(nextElement);

				for(EStructuralFeature feature : nextElement.eClass().getEAllStructuralFeatures()) {
					if(feature.isChangeable() && !(feature instanceof EReference && ((EReference)feature).isContainer())) {
						cc.append(SetCommand.create(editingDomain, nextElement, feature, conformance.eGet(feature)));
					}
				}
	...
	public boolean isRequired(String key, int kind) {
		return key == UMLViewsRepository.ProtocolStateMachine.isLeaf || key == UMLViewsRepository.ProtocolStateMachine.isAbstract || key == UMLViewsRepository.ProtocolStateMachine.isActive || key == UMLViewsRepository.ProtocolStateMachine.isReentrant || key == UMLViewsRepository.ProtocolStateMachine.region;
	}
	...
	public Diagnostic validate() {
		...
		if(IPropertiesEditionComponent.BATCH_MODE.equals(editing_mode)) {
			...
			validate = Diagnostician.INSTANCE.validate(protocolStateMachine);
