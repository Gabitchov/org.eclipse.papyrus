...
public final class TableInstanceUtils {
	 ...
	 * @return
	 */
	public static Set<ETypedElement> getAllETypedElement(final Table table, final List<Row> rowsToKeep, final List<? extends FacetSet> nextLoadedFacets) {
		final Set<ETypedElement> eTypedElements = new HashSet<ETypedElement>();
		final IFacetManager facetManager = IFacetManagerFactory.DEFAULT.getOrCreateFacetManager(new ResourceSetImpl());
		...
		for(Row row : rowsToKeep) {
			...
			try {
				final Set<ETypedElement> facetTEs = FacetUtils.getETypedElements(row.getElement(), ETypedElement.class, facetManager);
